// main.c — MemePanel「复古像素 RETRO-PIXEL」：System 7 / Win95 拟物灰
// 架构：UpdateLayeredWindow 整窗提交 32bpp 预乘 DIB；GDI+（flat C，见 gp.h）负责全部
//       矢量/文字/位图合成（GDI 直写 DIB 会破坏 alpha 通道，故一律 GDI+）。
//       主题：不透明经典 Mac 浅灰 #DDDDDD 面板 + 2px 纯黑硬边 double-bevel + 顶部
//       System 7 水平条纹标题栏 + 斜面按钮 + 表格式网格（1px 黑线）+ 真 Win95 滚动条。
//       全部直角；1px 线条用 GdipFillRectangleI 实心小矩形画（无 AA，像素干净）。
...
// ---- 「复古像素 RETRO-PIXEL」tokens（BRIEF.md）----
// 面板 #DDDDDD / 2px 纯黑硬边 + 1px 白(上左) 1px #808080(下右) 内斜面 / 0 圆角
// 强调 = Mac 系统蓝 #0000FF（激活标签/选中框），警示红 #FF0000（删除武装）
// 斜面按钮：常态 灰底+上左白+下右黑；按下/激活 反转 + 底 #BBBBBB；hover #E8E8E8 + 外点线框
static u32 ARGB(int a, int r, int g, int b)
{
    return ((u32)(a & 255) << 24) | ((u32)(r & 255) << 16) | ((u32)(g & 255) << 8) | (u32)(b & 255);
}
// 常量字面量（可作静态初始化器；ARGB 字节序 = A<<24|R<<16|G<<8|B）
#define RC_BLACK   0xFF000000u   // 纯黑（边框/文字）
#define RC_WHITE   0xFFFFFFFFu
#define RC_BLUE    0xFF0000FFu   // Mac 系统蓝（纯蓝）
#define RC_RED     0xFFFF0000u   // 警示红（删除武装）
#define RC_INK     0xFF000000u   // 全站黑字
#define RC_GRAY    0xFF808080u   // #808080 内斜面暗边
#define RC_PANEL   0xFFDDDDDDu   // #DDDDDD 面板灰
#define RC_HOVER   0xFFE8E8E8u   // #E8E8E8 hover 底
#define RC_PRESS   0xFFBBBBBBu   // #BBBBBB 按下/激活底
#define RC_TRACK   0xFFE0E0E0u   // #E0E0E0 滚动条 track
#define RC_TICK    0xFFC6C6C6u   // track 刻度线
#define RC_BLUE_OV 0xC80000FFu   // 选中整格蓝 α200（Mac 文件选中式）

// 常用实心画刷缓存（fr() 用；一次创建全程复用）
enum { BR_BLACK, BR_WHITE, BR_BLUE, BR_GRAY, BR_PANEL, BR_HOVER,
       BR_PRESS, BR_TRACK, BR_TICK, BR_RED, BR_BLUE_OV, BR_COUNT };
static GpSolidFill* g_br[BR_COUNT];
...
// 合成表面（窗口 = 面板 + 四周 S(BLEED) 透明出血；本主题无投影，硬直角）
static HDC     g_memDC;
...
// 字体 / 字符串格式
static GpFont* g_fText, * g_fUi, * g_fSmall, * g_fTiny, * g_fTinyB, * g_fMono, * g_fTitle;
...
static int  g_tblR;             // 网格表格右缘（滚动条贴其右侧）
#define TITLE_H 26               // 标题栏内容高（逻辑 px）
#define SB_W    16               // 滚动条宽（逻辑 px，Win95 = 16）
...
// ---- 复古像素核心工艺：实心小矩形（1px 线/斜面边，无 AA，像素干净）----
// [l,t)-(r,b) 半开区间填充；k = BR_* 画刷索引
static void fr(int l, int t, int r, int b, int k)
{
    if (r <= l || b <= t || !g_br[k]) return;
    GdipFillRectangleI(g_gfx, (GpBrush*)g_br[k], l, t, r - l, b - t);
}

// 任意色一次性填充（低频小元素用）
static void frc(int l, int t, int r, int b, u32 col)
{
    if (r <= l || b <= t) return;
    GpSolidFill* br = NULL;
    if (GdipCreateSolidFill(col, &br) == 0) {
        GdipFillRectangleI(g_gfx, (GpBrush*)br, l, t, r - l, b - t);
        GdipDeleteBrush((GpBrush*)br);
    }
}

// 斜面按钮（关键工艺）：0 常态 / 1 hover / 2 按下或激活（斜面反转）
// 常态   = 灰底 #DDDDDD + 上左 1px 白 + 下右 1px 黑
// 按下   = 底 #BBBBBB + 上左黑 + 下右白（内凹）
// hover  = 底 #E8E8E8 + 常态斜面 + 外 1px 黑点线框（Mac focus ring 之意）
static void bevel_cell(RECT rc, int state)
{
    fr(rc.left, rc.top, rc.right, rc.bottom,
       state == 2 ? BR_PRESS : (state == 1 ? BR_HOVER : BR_PANEL));
    int inv = (state == 2);
    fr(rc.left, rc.top, rc.right - 1, rc.top + 1,  inv ? BR_BLACK : BR_WHITE);  // 上
    fr(rc.left, rc.top, rc.left + 1, rc.bottom - 1, inv ? BR_BLACK : BR_WHITE); // 左
    fr(rc.left + 1, rc.bottom - 1, rc.right, rc.bottom, inv ? BR_WHITE : BR_BLACK); // 下
    fr(rc.right - 1, rc.top + 1, rc.right, rc.bottom, inv ? BR_WHITE : BR_BLACK);   // 右
    if (state == 1) {   // 外 1px 黑点线框（隔 1px 空隙，点距 3px）
        int l = rc.left - 2, t = rc.top - 2, r = rc.right + 2, b = rc.bottom + 2;
        for (int x = l; x < r; x += 3) {
            fr(x, t, x + 1, t + 1, BR_BLACK);
            fr(x, b - 1, x + 1, b, BR_BLACK);
        }
        for (int y = t + 3; y < b - 3; y += 3) {
            fr(l, y, l + 1, y + 1, BR_BLACK);
            fr(r - 1, y, r, y + 1, BR_BLACK);
        }
    }
}

// 凹槽字段（搜索框/状态栏）：上左 1px #808080 + 下右 1px 白
static void sunken_field(RECT rc, int kFill)
{
    fr(rc.left, rc.top, rc.right, rc.bottom, kFill);
    fr(rc.left, rc.top, rc.right - 1, rc.top + 1, BR_GRAY);
    fr(rc.left, rc.top, rc.left + 1, rc.bottom - 1, BR_GRAY);
    fr(rc.left + 1, rc.bottom - 1, rc.right, rc.bottom, BR_WHITE);
    fr(rc.right - 1, rc.top + 1, rc.right, rc.bottom - 1, BR_WHITE);
}

// 2px 纯黑硬边框（直角）
static void black_frame2(RECT rc)
{
    fr(rc.left, rc.top, rc.right, rc.top + 2, BR_BLACK);
    fr(rc.left, rc.bottom - 2, rc.right, rc.bottom, BR_BLACK);
    fr(rc.left, rc.top + 2, rc.left + 2, rc.bottom - 2, BR_BLACK);
    fr(rc.right - 2, rc.top + 2, rc.right, rc.bottom - 2, BR_BLACK);
}

// 1px 黑边框（直角）
static void black_frame1(RECT rc)
{
    fr(rc.left, rc.top, rc.right - 1, rc.top + 1, BR_BLACK);
    fr(rc.left, rc.bottom - 1, rc.right, rc.bottom, BR_BLACK);
    fr(rc.left, rc.top + 1, rc.left + 1, rc.bottom - 1, BR_BLACK);
    fr(rc.right - 1, rc.top + 1, rc.right, rc.bottom - 1, BR_BLACK);
}

// 蓝色 2px 选中框（扩 1px，不遮图：格子内容已内缩 2px）
static void blue_frame2(RECT rc)
{
    frc(rc.left, rc.top, rc.right, rc.top + 2, RC_BLUE);
    frc(rc.left, rc.bottom - 2, rc.right, rc.bottom, RC_BLUE);
    frc(rc.left, rc.top + 2, rc.left + 2, rc.bottom - 2, RC_BLUE);
    frc(rc.right - 2, rc.top + 2, rc.right, rc.bottom - 2, RC_BLUE);
}

// System 7 标题条纹：黑白 2px 相间，从 y0 画到 y1（x ∈ [l,r)）
static void stripes(int l, int t, int r, int b)
{
    for (int y = t; y < b; y += 2) {
        int y2 = y + 2; if (y2 > b) y2 = b;
        if (((y - t) / 2) & 1) fr(l, y, r, y2, BR_WHITE);
        else                   fr(l, y, r, y2, BR_BLACK);
    }
}

// 像素风小三角（滚动条箭头）：dir 0=上 1=下
static void tri_px(int cx, int cy, int dir)
{
    static const int w[5] = { 7, 5, 3, 1, 0 };
    for (int k = 0; k < 4; k++) {
        int ww = w[k];
        int y = dir ? cy + k : cy + (3 - k);
        fr(cx - ww / 2, y, cx - ww / 2 + ww, y + 1, BR_BLACK);
    }
}
...
// 静态件生成
// ============================================================
// 底图：不透明 #DDDDDD 面板 + 2px 纯黑硬边 + 1px 白高光(上左)/#808080(下右) double-bevel
//       + System 7 条纹标题栏 + 标题栏下 2px 黑分隔线。全部直角、纯像素行填充
//       （面板不透明：alpha 面内全 255，面外全 0，ULW 仍可用）。
static void gen_base(void)
{
    ...
    u32 cPanel = RC_PANEL, cBlack = RC_BLACK, cWhite = RC_WHITE, cGray = RC_GRAY;
    int tb = T + 3 + S(TITLE_H);              // 标题栏内容底（T+3 = 2px 黑边 + 1px 白高光）

    for (int y = T; y < B; y++) {
        u32* row = g_bits + (u64)y * W;
        if (y < T + 2 || y >= B - 2) {                    // 上下 2px 黑边
            for (int x = L; x < R; x++) row[x] = cBlack;
        } else if (y == T + 2) {                          // 上内 1px 白高光
            for (int x = L + 2; x < R - 2; x++) row[x] = cWhite;
        } else if (y == B - 3) {                          // 下内 1px #808080
            for (int x = L + 2; x < R - 2; x++) row[x] = cGray;
        } else if (y < tb) {                              // 标题条纹区（黑白 2px 相间，抵边）
            u32 c = (((y - (T + 3)) / 2) & 1) ? cWhite : cBlack;
            for (int x = L + 2; x < R - 2; x++) row[x] = c;
        } else if (y < tb + 2) {                          // 标题栏下 2px 黑分隔线
            for (int x = L; x < R; x++) row[x] = cBlack;
        } else {
            for (int x = L; x < R; x++) row[x] = cPanel;  // 面板灰
        }
    }
    // 左右 2px 黑边 + 内侧 1px 白(左)/#808080(右)（仅标题栏以下区域）
    for (int y = T; y < B; y++) {
        u32* row = g_bits + (u64)y * W;
        row[L] = cBlack; row[L + 1] = cBlack;
        row[R - 1] = cBlack; row[R - 2] = cBlack;
        if (y > T + 2 && y < B - 3 && y >= tb + 2) {
            row[L + 2] = cWhite; row[R - 3] = cGray;
        }
    }
...
static void rebuild_surface(void)
{
    ...
    if (g_gfx) {
        GdipSetSmoothingMode(g_gfx, 1);       // SmoothingModeNone：硬边（像素风，线条即像素）
        GdipSetInterpolationMode(g_gfx, GP_INTERP_BICUBIC);
        GdipSetTextRenderingHint(g_gfx, 1);   // SingleBitPerPixelGridFit：点阵感文字
    }
    if (g_base) gen_base();
}
...
// contain 绘制（保持比例、居中；rad>0 圆角 clip，rad<=0 直角矩形 clip）
...
static void panel_layout(HWND hwnd)
{
    ...
    int padX = S(16);
    int tb   = g_panel.top + 3 + S(TITLE_H);   // 标题栏内容底
    int top0 = tb + 2;                          // 标题栏 2px 黑分隔线以下，内容起点

    // 关闭钮：条纹标题栏右端小方块（黑边白底，hover 黑底）——命中区即方块本身
    int closeS = S(20);
    g_closeRc.right = g_panel.right - 3 - S(6);
    g_closeRc.left  = g_closeRc.right - closeS;
    g_closeRc.top    = g_panel.top + 3 + (S(TITLE_H) - closeS) / 2;
    g_closeRc.bottom = g_closeRc.top + closeS;

    // 搜索框：白底黑 2px 边凹槽，独占整行
    int sh = S(32);
    g_searchBox.left  = g_panel.left + padX;
    g_searchBox.top   = top0 + S(10);
    g_searchBox.right = g_panel.right - padX;
    g_searchBox.bottom = g_searchBox.top + sh;

    // 表格式网格几何（速发/管理共用）：滚动条 S(16) 贴表格右侧
    int sbw = S(SB_W);
    int chipH = S(24);
    if (g_mgr) {
        // 管理模式：操作行（斜面胶囊 + 斜面按钮）→ 大格表格 → 状态栏
        int opY = g_searchBox.bottom + S(12);
        g_cell = S(96); g_gap = 0;
        g_grid.left  = g_panel.left + padX;
        g_grid.top   = opY + chipH + S(12);
        g_grid.right = g_panel.right - padX;
        g_footer.left  = g_panel.left + padX;          // 状态栏四边齐全（left/right 别漏）
        g_footer.right = g_panel.right - padX;
        g_footer.bottom = g_panel.bottom - 3 - S(8);
        g_footer.top = g_footer.bottom - S(20);
        g_grid.bottom = g_footer.top - S(10);
    } else {
        // 速发模式：搜索 → 标签斜面按钮行 → 最近 7 格 → 表格网格 → 状态栏
        int chipY = g_searchBox.bottom + S(10);
        int ry = chipY + chipH + S(12);
        g_cell = S(56); g_gap = 0;
        for (int i = 0; i < 7; i++) {
            g_recent[i].left = g_panel.left + padX + i * (g_cell + S(6));
            g_recent[i].right = g_recent[i].left + g_cell;
            g_recent[i].top = ry;
            g_recent[i].bottom = ry + g_cell;
        }
        g_grid.left  = g_panel.left + padX;
        g_grid.top   = ry + g_cell + S(12);
        g_grid.right = g_panel.right - padX;
    ...
    // 列/行：表格宽 = 可用宽 - 滚动条；1px 外框内每列 g_cell 宽。
    // 余量 >= 2/3 格宽时收一列窄尾列（画内容），否则右侧留白填缝。
    {
        int avail  = g_grid.right - g_grid.left;
        int tableW = avail - sbw;
        int full   = (tableW - 1) / g_cell;
        if (full < 1) full = 1;
        int rem = tableW - 1 - full * g_cell;
        g_cols = (rem >= g_cell * 2 / 3) ? full + 1 : full;
        g_tblR = g_grid.left + tableW;
        int tableH = g_grid.bottom - g_grid.top;
        g_rows = (tableH - 1) / g_cell;
        if (g_rows < 1) g_rows = 1;
    }

    // 悬停大图（右下）：「窗中窗」浮窗 = 条纹标题 + 白底 + 黑 2px 边
    g_preview.right  = g_panel.right - padX;
    g_preview.bottom = g_footer.top - S(8);
    g_preview.left   = g_preview.right - (S(168) + 4);
    g_preview.top    = g_preview.bottom - (S(20) + S(152) + 4);
    clamp_first();
}
...
static int cell_from_point(POINT pt)
{
    ...
    // 表格内容从 1px 外框内起算（cell_from_point 与 draw_grid 必须同基准）
    int col = (pt.x - g_grid.left - 1) / g_cell; if (col < 0) col = 0;
    int row = (pt.y - g_grid.top - 1) / g_cell;  if (row < 0) row = 0;
    // 表格外（滚动条/右侧填缝/底行余量）与不足一格的残余条带不命中
    if (col >= g_cols || row >= g_rows) return -1;
    if (pt.x >= g_tblR - 1 || pt.y >= g_grid.bottom - 1) return -1;
...
// 绘制：动态件
// ============================================================
// 标题栏动态件：白底标题铭牌 + 黑字 + 右端关闭小方块（条纹底图已在 gen_base 烤好）
static void draw_titlebar(void)
{
    int tb   = g_panel.top + 3 + S(TITLE_H);
    // 铭牌：条纹上的白底块 + 黑粗标题（System 7 惯例）
    int pl = g_panel.left + S(10);
    int ph = S(18);
    int pt_ = g_panel.top + 3 + (S(TITLE_H) - ph) / 2;
    int tw = (int)gtext_w(APP_NAME, -1, g_fTitle) + S(10);
    fr(pl, pt_, pl + tw, pt_ + ph, BR_WHITE);
    RECT tr = { pl + S(5), pt_, pl + tw - S(2), pt_ + ph };
    gtext(APP_NAME, -1, g_fTitle, tr, g_sfL, RC_INK);

    // 关闭钮：黑边白底小方块，hover 黑底白 X（条纹中）
    RECT cb = g_closeRc;
    BOOL chov = PtInRect(&cb, g_mouse);
    fr(cb.left, cb.top, cb.right, cb.bottom, BR_WHITE);
    black_frame1(cb);
    GpPen* xp = NULL;
    if (GdipCreatePen1(chov ? RC_WHITE : RC_BLACK, 2.0f, GP_UNIT_PIXEL, &xp) == 0) {
        if (chov) fr(cb.left + 1, cb.top + 1, cb.right - 1, cb.bottom - 1, BR_BLACK);
        int m = S(6);
...
{
    RECT rc = g_searchBox;
    // 白底黑 2px 边凹槽：内凹斜面 = 上左 1px #808080、下右 1px 白
    fr(rc.left, rc.top, rc.right, rc.bottom, BR_BLACK);
    fr(rc.left + 2, rc.top + 2, rc.right - 2, rc.bottom - 2, BR_WHITE);
    fr(rc.left + 2, rc.top + 2, rc.right - 2, rc.top + 3, BR_GRAY);
    fr(rc.left + 2, rc.top + 3, rc.left + 3, rc.bottom - 3, BR_GRAY);

    // 放大镜（黑，点阵感）
    GpPen* mp = NULL;
    if (GdipCreatePen1(RC_BLACK, 1.6f, GP_UNIT_PIXEL, &mp) == 0) {
        int cx = rc.left + S(16), cy = (rc.top + rc.bottom) / 2 - S(1);
        GdipDrawEllipseI(g_gfx, mp, cx - S(5), cy - S(6), S(9), S(9));
    ...
    if (g_search[0] || g_complen) {
        ...
        if (g_caret < g_slen) {
            ...
            gtext(g_search + g_caret, g_slen - g_caret, g_fText, rr2, g_sfL, RC_INK);
        }
    }
    // caret（黑竖线，空输入也显示）
    if (g_caretOn && !g_complen) {
        ...
        int cxx = tx + (int)preW;
        GpPen* cp = NULL;
        if (GdipCreatePen1(RC_BLACK, 1.6f, GP_UNIT_PIXEL, &cp) == 0) {
...
{
    // 最近快捷行：白底 1px 黑边小方格（桌面图标感）；hover = 蓝 2px 框（扩 1px 不遮图）
    for (int i = 0; i < 7; i++) {
        int ei = (i < g_nRecent && g_recentIdx[i] < g_store.count) ? g_recentIdx[i] : -1;
        if (ei < 0) continue;
        RECT rc = g_recent[i];
        BOOL hov = PtInRect(&rc, g_mouse);
        fr(rc.left, rc.top, rc.right, rc.bottom, BR_BLACK);       // 1px 黑边
        fr(rc.left + 1, rc.top + 1, rc.right - 1, rc.bottom - 1, BR_WHITE);
        if (hov && g_anim.ei == ei && g_anim.pb) {
            draw_image_contain(g_anim.pb[g_anim.cur], g_anim.w, g_anim.h,
                               (RECT){ rc.left + 3, rc.top + 3, rc.right - 3, rc.bottom - 3 }, 0);
        } else {
            int iw, ih;
        ...
        if (hov) blue_frame2((RECT){ rc.left - 1, rc.top - 1, rc.right + 1, rc.bottom + 1 });
    }
}

// 标签 = 斜面小按钮；激活 = 按下态（反转斜面 + #BBBBBB 底）+ Mac 蓝字
static void draw_chip(int* px, int y, const wchar_t* txt, int id)
{
    float w = gtext_w(txt, -1, g_fUi) + S(20);
    int h = S(24);
    ...
    bevel_cell(rc, active ? 2 : (hov ? 1 : 0));
    gtext(txt, -1, g_fUi, rc, g_sfC, active ? RC_BLUE : RC_INK);
...
{
    // 空态：表格区 1px 黑点线框（拖放目标）+ 居中黑字
    RECT rc = { g_grid.left, g_grid.top, g_tblR - 1, g_grid.bottom - 1 };
    fr(rc.left + 1, rc.top + 1, rc.right - 1, rc.bottom - 1, BR_WHITE);
    for (int x = rc.left; x < rc.right; x += 3) {
        fr(x, rc.top, x + 1, rc.top + 1, BR_BLACK);
        fr(x, rc.bottom - 1, x + 1, rc.bottom, BR_BLACK);
    }
    for (int y = rc.top; y < rc.bottom; y += 3) {
        fr(rc.left, y, rc.left + 1, y + 1, BR_BLACK);
        fr(rc.right - 1, y, rc.right, y + 1, BR_BLACK);
    }
    int cy = (rc.top + rc.bottom) / 2;
    gtext(L"把表情图拖到这里", -1, g_fUi,
          (RECT){ rc.left, cy - S(12), rc.right, cy + S(8) }, g_sfC, RC_INK);
}

// 真 Win95 滚动条（常驻）：#E0E0E0 track + 竖向刻度线、斜面 thumb 带 3 条刻痕、上下箭头黑三角
// 内容全放下时 thumb 满轨（经典常驻滚动条；本主题的签名件，不做按需隐藏）
static void draw_scrollbar(int vis)
{
    if (g_nFilt <= 0) return;
    int sbw = S(SB_W);
    int x0 = g_tblR, x1 = g_grid.right;
    int top = g_grid.top, bot = g_grid.bottom;
    if (x1 - x0 < S(10)) return;

    int abH = sbw;                                    // 上下箭头钮 = 正方
    // track
    fr(x0, top, x1, bot, BR_TRACK);
    for (int y = top + abH + 2; y < bot - abH - 2; y += 4)   // 竖向刻度线
        fr(x0 + 3, y, x1 - 3, y + 1, BR_TICK);
    // thumb
    int trackTop = top + abH, trackBot = bot - abH;
    int trackH = trackBot - trackTop;
    int thumbH = trackH * vis / g_nFilt;
    if (thumbH < S(28)) thumbH = S(28);
    int maxF = g_nFilt - vis;
    int thumbY = trackTop + (int)((trackH - thumbH) * (maxF ? (double)g_first / maxF : 0));
    RECT th = { x0, thumbY, x1, thumbY + thumbH };
    if (th.bottom > trackBot) { th.bottom = trackBot; th.top = th.bottom - thumbH; }
    if (th.top < trackTop) th.top = trackTop;
    bevel_cell(th, 0);
    int cx = (x0 + x1) / 2, cy = (th.top + th.bottom) / 2;   // 3 条刻痕线
    fr(cx - S(4), cy - 3, cx + S(4), cy - 2, BR_GRAY);
    fr(cx - S(4), cy,     cx + S(4), cy + 1, BR_GRAY);
    fr(cx - S(4), cy + 2, cx + S(4), cy + 3, BR_GRAY);
    // 上下箭头钮（斜面 + 黑像素三角）
    RECT up = { x0, top, x1, top + abH }, dn = { x0, bot - abH, x1, bot };
    bevel_cell(up, 0);
    bevel_cell(dn, 0);
    tri_px(cx, (up.top + up.bottom) / 2 - 2, 0);
    tri_px(cx, (dn.top + dn.bottom) / 2 - 2, 1);
}

static void draw_grid(void)
{
    if (g_nFilt == 0) { draw_empty(); return; }
    // 表格容器：白底 + 1px 黑外框；cell 间 1px 黑线（表格感），0px 间距 0 圆角
    int left = g_grid.left, top = g_grid.top, tblR = g_tblR, bot = g_grid.bottom;
    fr(left, top, tblR, bot, BR_WHITE);

    int nameH = g_mgr ? S(18) : 0;                    // 管理模式：格底文件名条（Finder 式）
    for (int k = 0; k < g_cols * g_rows; k++) {
        int idx = g_first + k;
        if (idx >= g_nFilt) break;
        int col = k % g_cols, row = k / g_cols;
        RECT rc = { left + 1 + col * g_cell, top + 1 + row * g_cell, 0, 0 };
        rc.right = rc.left + g_cell; rc.bottom = rc.top + g_cell;
        if (rc.right > tblR - 1) rc.right = tblR - 1;  // 窄尾列收在框内
        if (rc.bottom > bot - 1) rc.bottom = bot - 1;
        BOOL hov = (idx == g_hover);
        BOOL sel = g_mgr && sel_has(g_filt[idx]);
        Entry* e = &g_store.entries[g_filt[idx]];

        // 图片（内缩 2px，不碰表格线）
        RECT ir = { rc.left + 2, rc.top + 2, rc.right - 2, rc.bottom - 2 - nameH };
        int iw, ih;
        ...
        // 管理模式：格底文件名（黑字，选中反白）
        if (g_mgr) {
            RECT nr = { rc.left + 2, rc.bottom - nameH - 1, rc.right - 2, rc.bottom - 1 };
            gtext(e->name, -1, g_fTiny, nr, g_sfC, RC_INK);
        }

        // 选中：整格蓝 α200（Mac 文件选中式）+ 文件名反白
        if (sel) {
            frc(rc.left + 1, rc.top + 1, rc.right - 1, rc.bottom - 1, RC_BLUE_OV);
            RECT nr = { rc.left + 2, rc.bottom - nameH - 1, rc.right - 2, rc.bottom - 1 };
            gtext(e->name, -1, g_fTiny, nr, g_sfC, RC_WHITE);
        }

        // GIF 角标：黑底白字方块
        if (is_gif_name(e->name)) {
            RECT gc = { rc.right - S(25), ir.bottom - S(14), rc.right - 3, ir.bottom - 1 };
            fr(gc.left, gc.top, gc.right, gc.bottom, BR_BLACK);
            RECT gt = gc; gt.top -= S(1);
            gtext(L"GIF", -1, g_fTiny, gt, g_sfC, RC_WHITE);
        }

        // hover：蓝 2px 框（扩 1px 不遮图）+ 底部黑条白字文件名（状态栏样式，速发模式）
        if (hov) {
            RECT fb = { rc.left - 1, rc.top - 1, rc.right + 1, rc.bottom + 1 };
            if (fb.left < left) fb.left = left;
            if (fb.right > tblR) fb.right = tblR;
            if (fb.top < top) fb.top = top;
            if (fb.bottom > bot) fb.bottom = bot;
            blue_frame2(fb);
            if (!g_mgr) {   // 管理模式文件名常驻（下方条），黑条只在速发出现
                RECT nb = { rc.left + 1, rc.bottom - S(16), rc.right - 1, rc.bottom - 1 };
                fr(nb.left, nb.top, nb.right, nb.bottom, BR_BLACK);
                RECT nt = nb; nt.top -= S(1);
                gtext(e->name, -1, g_fTiny, nt, g_sfC, RC_WHITE);
            }
        }
    }

    // 表格线：竖线在每列左缘（k=1..cols-1），横线在每行上缘（k=1..rows-1）
    for (int k = 1; k < g_cols; k++)
        fr(left + 1 + k * g_cell, top, left + 1 + k * g_cell + 1, bot, BR_BLACK);
    for (int k = 1; k < g_rows; k++)
        fr(left, top + 1 + k * g_cell, tblR, top + 1 + k * g_cell + 1, BR_BLACK);
    // 1px 黑外框
    fr(left, top, tblR, top + 1, BR_BLACK);
    fr(left, bot - 1, tblR, bot, BR_BLACK);
    fr(left, top + 1, left + 1, bot - 1, BR_BLACK);
    fr(tblR - 1, top + 1, tblR, bot - 1, BR_BLACK);

    draw_scrollbar(g_cols * g_rows);
}

// 状态栏：灰槽（凹斜面）内黑字；速发=悬停文件名+N items，管理=状态行+选中计数
static void draw_footer(void)
{
    RECT rc = g_footer;
    sunken_field(rc, BR_PANEL);
    ...
    if (!g_mgr) {
        if (g_hover >= 0 && g_hover < g_nFilt) {
            ...
            RECT lr = { rc.left + S(8), rc.top, rc.right - S(90), rc.bottom };
            gtext(line, -1, g_fSmall, lr, g_sfL, RC_INK);
        }
    } else if (g_status[0]) {
        gtext(g_status, -1, g_fSmall,
              (RECT){ rc.left + S(8), rc.top, rc.right - S(160), rc.bottom },
              g_sfL, RC_INK);
    }
    // 右端计数（Consolas）
    wchar_t cnt[48];
    if (g_mgr) wnsprintfW(cnt, 48, L"选中 %d · 共 %d", g_nSel, g_nFilt);
    else       wnsprintfW(cnt, 48, L"%d items", g_nFilt);
    gtext(cnt, -1, g_fMono,
          (RECT){ rc.right - S(88), rc.top, rc.right - S(6), rc.bottom },
          g_sfR, RC_INK);
}

// 大图预览 = 「窗中窗」浮窗：白底 + 黑 2px 边 + 条纹标题栏（小「Preview」铭牌）
static void draw_preview(void)
{
    if (g_previewIdx < 0 || g_previewIdx >= g_nFilt) return;
    RECT rc = g_preview;
    fr(rc.left, rc.top, rc.right, rc.bottom, BR_WHITE);
    // 条纹标题
    int th = S(20);
    stripes(rc.left + 2, rc.top + 2, rc.right - 2, rc.top + th);
    int pl = rc.left + S(6);
    int pw = (int)gtext_w(L"Preview", -1, g_fTinyB) + S(8);
    int ph = S(12);
    int py = rc.top + 2 + (th - 4 - ph) / 2;
    fr(pl, py, pl + pw, py + ph, BR_WHITE);
    gtext(L"Preview", -1, g_fTinyB, (RECT){ pl + S(3), py - 1, pl + pw - S(2), py + ph }, g_sfL, RC_INK);
    // 图
    int ei = g_filt[g_previewIdx];
    RECT ir = { rc.left + 2, rc.top + th, rc.right - 2, rc.bottom - 2 };
    int iw, ih;
...
// 管理模式：操作行按钮（右端对齐排布，rect 记录命中）；全斜面，武装删除=红底白字
static void draw_mgr_button(int* pright, int y, const wchar_t* txt, int id, BOOL danger)
{
    ...
    float w = gtext_w(id == BTN_DELETE ? L"确认删除" : txt, -1, g_fUi) + S(20);
    RECT rc = { *pright - (int)w, y, *pright, y + S(24) };
    BOOL hov = PtInRect(&rc, g_mouse);
    BOOL on = (id == BTN_DELETE) ? g_delArm : FALSE;

    if (on) {   // 武装态：红底白字 + 反转斜面（静态警示）
        fr(rc.left, rc.top, rc.right, rc.bottom, BR_RED);
        fr(rc.left, rc.top, rc.right - 1, rc.top + 1, BR_BLACK);
        fr(rc.left, rc.top, rc.left + 1, rc.bottom - 1, BR_BLACK);
        fr(rc.left + 1, rc.bottom - 1, rc.right, rc.bottom, BR_WHITE);
        fr(rc.right - 1, rc.top + 1, rc.right, rc.bottom, BR_WHITE);
    } else {
        bevel_cell(rc, hov ? 1 : 0);
    }
    u32 tc = on ? RC_WHITE : (danger && hov ? RC_RED : RC_INK);
    gtext(txt, -1, g_fUi, rc, g_sfC, tc);
    g_btnRc[id] = rc;
    *pright = rc.left - S(8);
}
...
{
    // 操作行：左标签斜面按钮（选中>0 时点击=批量打标）+ 右按钮组
    int opY = g_searchBox.bottom + S(12);
    g_nChips = 0;
    ...
    // 按钮组右界贴面板右缘（关闭钮已移入标题栏，不再冲突）
    int pr = g_panel.right - S(16);
    ...
    // 网格（draw_grid 已按 g_mgr 画选中态）+ 状态栏
    draw_grid();
    draw_footer();
}
...
static void ime_update_position(HWND hwnd)
{
    ...
    if (!im) return;
    float preW = g_caret ? gtext_w(g_search, g_caret, g_fText) : 0.0f;
    int cxx = g_searchBox.left + S(32) + (int)preW;
...
// ============================================================
// 图标（复古像素风：灰底方窗 + 黑边 + 条纹标题 + 黑笑脸；GDI 画在不透明 icon DIB）
// ============================================================
static void fill_rc(HDC dc, int l, int t, int r, int b, HBRUSH br)
{
    RECT rc = { l, t, r, b };
    FillRect(dc, &rc, br);
}
...
static HICON make_app_icon(void)
{
    ...
    if (color && mask && colorBits && maskBits) {
        ...
        for (int i = 0; i < 64 * 64; i++) px[i] = 0xFFDDDDDDu;
        HDC mdc = CreateCompatibleDC(sdc);
        HGDIOBJ old = SelectObject(mdc, color);
        HBRUSH bb = CreateSolidBrush(RGB(0, 0, 0));
        HBRUSH wb = CreateSolidBrush(RGB(255, 255, 255));
        // 3px 黑硬边方框
        fill_rc(mdc, 0, 0, 64, 3, bb);   fill_rc(mdc, 0, 61, 64, 64, bb);
        fill_rc(mdc, 0, 3, 3, 61, bb);   fill_rc(mdc, 61, 3, 64, 61, bb);
        // 条纹标题栏（黑白 2px 相间，y 3..21）
        for (int y = 3; y < 21; y += 4) fill_rc(mdc, 3, y, 61, y + 2, bb);
        // 标题栏下黑线
        fill_rc(mdc, 3, 21, 61, 23, bb);
        // 黑点眼 + 弧线笑（像素脸）
        fill_rc(mdc, 18, 32, 24, 38, bb);
        fill_rc(mdc, 40, 32, 46, 38, bb);
        HPEN bp = CreatePen(PS_SOLID, 4, RGB(0, 0, 0));
        HGDIOBJ op = SelectObject(mdc, bp);
        SelectObject(mdc, GetStockObject(NULL_BRUSH));
        Arc(mdc, 18, 38, 46, 58, 22, 48, 42, 48);
        SelectObject(mdc, op);
        DeleteObject(bb); DeleteObject(wb); DeleteObject(bp);
...
static void shot_run(HWND hwnd)
{
    ...
    if (cl_arg(L"-hover") && g_nFilt > 0) {
        ...
        g_mouse.x = g_grid.left + 1 + col * g_cell + g_cell / 2;   // 表格内容自 1px 外框内起算
        g_mouse.y = g_grid.top + 1 + row * g_cell + g_cell / 2;
...
void entry(void)
{
    ...
    // 字体（全站 Tahoma / 计数 Consolas，点阵感）/ 格式 / 画刷缓存
    HDC fdc = GetDC(NULL);
    g_fText  = make_font(fdc, S(12), FW_NORMAL, L"Tahoma");    // 搜索输入
    g_fUi    = make_font(fdc, S(12), FW_NORMAL, L"Tahoma");    // 按钮/标签/正文
    g_fSmall = make_font(fdc, S(10), FW_NORMAL, L"Tahoma");    // 状态栏小字
    g_fTiny  = make_font(fdc, S(9),  FW_NORMAL, L"Tahoma");    // 名字条/GIF 角标
    g_fTinyB = make_font(fdc, S(9),  FW_BOLD,   L"Tahoma");    // 浮窗小标题
    g_fMono  = make_font(fdc, S(10), FW_NORMAL, L"Consolas");  // 计数
    g_fTitle = make_font(fdc, S(12), FW_BOLD,   L"Tahoma");    // 标题栏
    ReleaseDC(NULL, fdc);
    ...
    {
        static const u32 brCol[BR_COUNT] = {
            RC_BLACK, RC_WHITE, RC_BLUE, RC_GRAY, RC_PANEL, RC_HOVER,
            RC_PRESS, RC_TRACK, RC_TICK, RC_RED, RC_BLUE_OV
        };
        for (int i = 0; i < BR_COUNT; i++) GdipCreateSolidFill(brCol[i], &g_br[i]);
    }
