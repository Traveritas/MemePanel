// main.c — MemePanel「液态玻璃 LIQUID」：材质最大化变体（Vista Aero 魂，GDI 躯壳）
// 架构：UpdateLayeredWindow 整窗提交 32bpp 预乘 DIB；GDI+（flat C，见 gp.h）负责全部
//       矢量/文字/位图合成（GDI 直写 DIB 会破坏 alpha 通道，故一律 GDI+）。
//       静态底图（垂直三段液态渐变 + 顶部径向白光 + 折射光带 + 镜面 streak + 三层色散边）
//       resize 时预渲染一次；每帧 = memcpy 底图 + 动态件绘制 + 一次 ULW 提交。
...
// ---- 「液态玻璃 LIQUID」tokens（BRIEF.md）----
// 底盘三段：顶 rgba(150,170,210,.55) → 中 rgba(60,70,95,.72) → 底 rgba(25,30,45,.85)
// 圆角：外壳 24 / 格子 14 / 胶囊 999；强调 #6FD3FF ↔ #B28BFF；色散暖边 #FFB86B
#define T_ACC_R  111                // ACCENT  #6FD3FF 青蓝
#define T_ACC_G  211
#define T_ACC_B  255
#define T_ACC2_R 178                // ACCENT2 #B28BFF 紫
#define T_ACC2_G 139
#define T_ACC2_B 255
#define T_WRM_R  255                // WARM    #FFB86B 色散暖端
#define T_WRM_G  184
#define T_WRM_B  107
...
#define C_INK(a)      ARGB(a, 240, 247, 255)              // 主文字 rgba(240,247,255,.95)
#define C_ACCENT(a)   ARGB(a, T_ACC_R, T_ACC_G, T_ACC_B)
#define C_ACCENT2(a)  ARGB(a, T_ACC2_R, T_ACC2_G, T_ACC2_B)
#define C_WARM(a)     ARGB(a, T_WRM_R, T_WRM_G, T_WRM_B)
#define RAD_SHELL     S(24)                               // 外壳圆角
#define RAD_CELL      S(14)                               // 格子/字段圆角
...
static GpBitmap* g_cellTex, * g_cellTexHot;   // 64x64 液态格底（normal/hover 两档亮度）
static u8*   g_cellTexBuf, * g_cellTexHotBuf;
static GpBitmap* g_bandTex;                   // 8x256 折射光带剖面（sin 衰减白）
static u8*   g_bandTexBuf;

// 字体 / 字符串格式 / 笔刷
static GpFont* g_fText, * g_fUi, * g_fUiB, * g_fSmall, * g_fTiny, * g_fMono;
...
static RECT g_grid, g_dock, g_footer, g_preview;
...
// 无 CRT 的 cos（生成期纹理用，泰勒 8 阶，|误差| < 1e-6 @ [-π,π]）
static float cosf_(float x)
{
    while (x > 6.2831853f)  x -= 6.2831853f;
    while (x < 0.0f)        x += 6.2831853f;
    if (x > 3.1415927f)     x -= 6.2831853f;
    float x2 = x * x, x4 = x2 * x2, x8 = x4 * x4;
    return 1.0f - x2 / 2.0f + x4 / 24.0f - x4 * x2 / 720.0f + x8 / 40320.0f - x8 * x2 / 3628800.0f;
}
...
// 液态文字：1px 下移暗影 pass + 亮色 pass（玻璃上浮起的字）
static void gtext_shadow(const wchar_t* s, int len, GpFont* f, RECT rc,
                         GpStringFormat* sf, u32 argb)
{
    RECT d = rc; d.top += S(1); d.bottom += S(1);
    gtext(s, len, f, d, sf, ARGB(210, 6, 8, 16));
    gtext(s, len, f, rc, sf, argb);
}

// 线渐变填充圆角矩形：clip 到路径后走 FillRectangleI——FillPath × LineBrush ×
// 32PARGB scan0 表面是实测不渲染的组合（实底画刷无此问题），一律绕开
static void fill_rr_gradient(RECT rc, int rad, GpLineGradient* lg)
{
    GpPath* p = rr_path(rc.left, rc.top, rc.right, rc.bottom, rad);
    if (p) {
        GdipSetClipPath(g_gfx, p, GP_CM_INTERSECT);
        GdipFillRectangleI(g_gfx, (GpBrush*)lg, rc.left, rc.top,
...
// 液态玻璃格底（normal/hover/active 三态）：
//   normal/hover = 预生成垂直渐变贴图（顶亮→底暗，hover 换更亮一档）+ 白边 + 底缘受光线
//   active       = 青→紫渐变 + 上缘白高光弧（胶囊 ∩ 椭圆 clip 出的上 40% 白渐变）
static void glass_cell(RECT rc, int rad, int state, float baseA)
{
    int w = rc.right - rc.left, h = rc.bottom - rc.top;
    if (w < 2 || h < 2) return;
    (void)baseA;
    if (state == 2) {   // 激活：青蓝 -> 紫渐变
        GpLineGradient* lg = NULL;
        GpPoint p1 = { rc.left, rc.bottom - S(4) }, p2 = { rc.right, rc.top + S(4) };
        if (GdipCreateLineBrushI(&p1, &p2, C_ACCENT(92), C_ACCENT2(72), GP_WRAP_TILE, &lg) == 0) {
            fill_rr_gradient(rc, rad, lg);
            GdipDeleteBrush((GpBrush*)lg);
        }
        // 上缘白高光弧：胶囊路径 ∩ 大椭圆（下弧压在 45% 高度）内填垂直白渐变
        GpPath* cap = rr_path(rc.left, rc.top, rc.right, rc.bottom, rad);
        GpPath* el = NULL;
        GdipCreatePath(GP_FILL_WINDING, &el);
        if (el) {
            int ew = w + 2 * (w / 5), eh = h + h / 8;
            int ex = rc.left - (w / 5), ey = rc.top + h * 45 / 100 - eh;
            GdipAddPathEllipseI(el, ex, ey, ew, eh);
        }
        if (cap && el) {
            GdipSetClipPath(g_gfx, cap, 0);
            GdipSetClipPath(g_gfx, el, GP_CM_INTERSECT);
            GpLineGradient* hl = NULL;
            GpPoint q1 = { rc.left + w / 2, rc.top }, q2 = { rc.left + w / 2, rc.top + h * 2 / 5 };
            if (GdipCreateLineBrushI(&q1, &q2, ARGB(102, 255, 255, 255), 0,
                                     GP_WRAP_TILE, &hl) == 0) {
                GdipFillRectangleI(g_gfx, (GpBrush*)hl, rc.left, rc.top, w, h);
                GdipDeleteBrush((GpBrush*)hl);
            }
            GdipResetClip(g_gfx);
        }
        if (cap) GdipDeletePath(cap);
        if (el)  GdipDeletePath(el);
    } else {            // 液态贴图底（clip 到圆角，拉伸铺满）
        GpPath* p = rr_path(rc.left, rc.top, rc.right, rc.bottom, rad);
        if (p) {
            GdipSetClipPath(g_gfx, p, 0);
            GdipDrawImageRectI(g_gfx, (GpImage*)(state == 1 ? g_cellTexHot : g_cellTex),
                               rc.left, rc.top, w, h);
            GdipResetClip(g_gfx);
            GdipDeletePath(p);
        }
    ...
    // 边
    GpPen* pen = NULL;
    u32 pc = state == 2 ? C_ACCENT(170)
            : state == 1 ? ARGB(132, 255, 255, 255) : ARGB(30, 255, 255, 255);
    if (GdipCreatePen1(pc, 1.0f, GP_UNIT_PIXEL, &pen) == 0) {
        stroke_rr(rc.left, rc.top, rc.right, rc.bottom, rad, pen);
        GdipDeletePen(pen);
    }
    // hover：底缘 1px 受光亮线（液滴底部聚光）+ 顶缘高光
    if (state == 1 && rad * 2 < w) {
        GpPen* pl = NULL;
        if (GdipCreatePen1(ARGB(150, 255, 255, 255), 1.0f, GP_UNIT_PIXEL, &pl) == 0) {
            GdipDrawLineI(g_gfx, pl, rc.left + rad + 2, rc.bottom - 2,
                          rc.right - rad - 2, rc.bottom - 2);
            GdipDeletePen(pl);
        }
        GpPen* pt = NULL;
        if (GdipCreatePen1(ARGB(70, 255, 255, 255), 1.0f, GP_UNIT_PIXEL, &pt) == 0) {
            GdipDrawLineI(g_gfx, pt, rc.left + rad + 2, rc.top + 2,
                          rc.right - rad - 2, rc.top + 2);
            GdipDeletePen(pt);
        }
    }
}
...
static void free_statics(void)
{
    ...
    if (g_cellTex)     { GdipDisposeImage((GpImage*)g_cellTex);      g_cellTex = NULL; }
    if (g_cellTexHot)  { GdipDisposeImage((GpImage*)g_cellTexHot);   g_cellTexHot = NULL; }
    if (g_cellTexBuf)     { HeapFree(GetProcessHeap(), 0, g_cellTexBuf);     g_cellTexBuf = NULL; }
    if (g_cellTexHotBuf)  { HeapFree(GetProcessHeap(), 0, g_cellTexHotBuf); g_cellTexHotBuf = NULL; }
    if (g_bandTex)     { GdipDisposeImage((GpImage*)g_bandTex);      g_bandTex = NULL; }
    if (g_bandTexBuf)  { HeapFree(GetProcessHeap(), 0, g_bandTexBuf); g_bandTexBuf = NULL; }
}

// 64x64 液态格底：白色垂直渐变（topA -> botA），预乘、拉伸到格子
static void make_cell_tex(GpBitmap** out, u8** bufOut, float topA, float botA)
{
    const int N = 64;
    u8* buf = (u8*)HeapAlloc(GetProcessHeap(), 0, (u64)N * N * 4);
    if (!buf) return;
    ...
    for (int y = 0; y < N; y++) {
        float t = (float)y / (N - 1);
        int a = (int)((topA + (botA - topA) * t) * 255);
        if (a < 0) a = 0; if (a > 255) a = 255;
        u32 line = ARGB(a, a, a, a);      // 白色预乘
        for (int x = 0; x < N; x++) px[y * N + x] = line;
    }
    GdipCreateBitmapFromScan0(N, N, N * 4, GP_PF_32PARGB, buf, out);
    *bufOut = buf;
}

// 8x256 折射光带剖面：sin 衰减白（cos^2 钟形，两端羽化为 0）
static void make_band_tex(GpBitmap** out, u8** bufOut, float peak)
{
    const int W = 8, H = 256;
    u8* buf = (u8*)HeapAlloc(GetProcessHeap(), 0, (u64)W * H * 4);
    if (!buf) return;
    u32* px = (u32*)buf;
    for (int y = 0; y < H; y++) {
        float t = (float)y / (H - 1);
        float s = 0.5f - 0.5f * cosf_(t * 6.2831853f);   // sin^2 钟
        int a = (int)(peak * s * 255);
        if (a < 0) a = 0; if (a > 255) a = 255;
        u32 line = ARGB(a, a, a, a);
        for (int x = 0; x < W; x++) px[y * W + x] = line;
    }
    GdipCreateBitmapFromScan0(W, H, W * 4, GP_PF_32PARGB, buf, out);
    *bufOut = buf;
}

// 镜面 streak：沿 (A->B) 轴的平行四边形亮痕，垂直方向透明->亮->透明（两半各一次 LineBrush）
static void streak(float ax, float ay, float bx, float by, float w, int alpha)
{
    float dx = bx - ax, dy = by - ay;
    float len = xsqrt(dx * dx + dy * dy);
    if (len < 1.0f) return;
    dx /= len; dy /= len;
    float qx = -dy * w * 0.5f, qy = dx * w * 0.5f;   // 垂直于轴向的半宽向量
    // 渐变轴取在 streak 中点（可见区域内）——GDI+ 钳制线渐变在轴完全离屏时
    // 会整片渲染为空，轴放在形状穿过的位置才可靠
    float mx = (ax + bx) * 0.5f, my = (ay + by) * 0.5f;
    for (int side = 0; side < 2; side++) {
        GpLineGradient* lg = NULL;
        GpPoint p1, p2;
        u32 cb = ARGB(alpha, 255, 255, 255);
        if (side == 0) {  // 轴线(亮) -> +perp(透明)
            p1.x = (int)mx; p1.y = (int)my;
            p2.x = (int)(mx + qx); p2.y = (int)(my + qy);
            if (GdipCreateLineBrushI(&p1, &p2, cb, 0, GP_WRAP_TILE, &lg)) continue;
        } else {          // -perp(透明) -> 轴线(亮)
            p1.x = (int)(mx - qx); p1.y = (int)(my - qy);
            p2.x = (int)mx; p2.y = (int)my;
            if (GdipCreateLineBrushI(&p1, &p2, 0, cb, GP_WRAP_TILE, &lg)) continue;
        }
        GpPath* q = NULL;
        if (GdipCreatePath(GP_FILL_WINDING, &q) == 0) {
            int a1x, a1y, b1x, b1y, a2x, a2y, b2x, b2y;
            if (side == 0) {  // [A+perp, B+perp, B, A]
                a1x = (int)(ax + qx); a1y = (int)(ay + qy);
                b1x = (int)(bx + qx); b1y = (int)(by + qy);
                a2x = (int)ax; a2y = (int)ay; b2x = (int)bx; b2y = (int)by;
            } else {          // [A-perp, B-perp, B, A]
                a1x = (int)(ax - qx); a1y = (int)(ay - qy);
                b1x = (int)(bx - qx); b1y = (int)(by - qy);
                a2x = (int)ax; a2y = (int)ay; b2x = (int)bx; b2y = (int)by;
            }
            GdipStartPathFigure(q);
            GdipAddPathLineI(q, a1x, a1y, b1x, b1y);
            GdipAddPathLineI(q, b1x, b1y, b2x, b2y);
            GdipAddPathLineI(q, b2x, b2y, a2x, a2y);
            GdipClosePathFigure(q);
            // FillPath × LineBrush 不渲染（见 fill_rr_gradient 注释）：clip 四边形后 FillRectangleI
            int xs[4] = { a1x, b1x, a2x, b2x }, ys[4] = { a1y, b1y, a2y, b2y };
            ...
            GdipSetClipPath(g_gfx, q, GP_CM_INTERSECT);   // 与面板 clip 相交
            GdipFillRectangleI(g_gfx, (GpBrush*)lg, bx0, by0, bx1 - bx0 + 1, by1 - by0 + 1);
            GdipResetClip(g_gfx);
            GdipDeletePath(q);
        }
        GdipDeleteBrush((GpBrush*)lg);
...
// 底图（液态玻璃）：垂直三段渐变(150,170,210,.55 -> 60,70,95,.72 -> 25,30,45,.85)
// + 顶部径向白光 peak .22 + 折射光带 + 镜面 streak×2 + 三层色散描边（左冷右暖）
static void gen_base(void)
{
    ...
    float rad = (float)RAD_SHELL;
    float hw = pw / 2, hh = ph / 2;
    // 顶部径向白光（peak 0.22）
    float hcx = L + pw * 0.10f, hcy = T - ph * 0.06f;
    float hrx = pw * 0.92f, hry = pw * 190.0f / 700.0f;
    ...
    for (int y = 0; y < H; y++) {
        ...
        for (int x = 0; x < W; x++) {
            ...
            if (t < 0.5f) {    // 顶 -> 中
                float u = t * 2.0f;
                r = 150 + (60 - 150) * u;  g = 170 + (70 - 170) * u;  b = 210 + (95 - 210) * u;
                a = 0.55f + (0.72f - 0.55f) * u;
            } else {          // 中 -> 底
                float u = (t - 0.5f) * 2.0f;
                r = 60 + (25 - 60) * u;   g = 70 + (30 - 70) * u;   b = 95 + (45 - 95) * u;
                a = 0.72f + (0.85f - 0.72f) * u;
            }
            float ex = (fx - hcx) / hrx, ey = (fy - hcy) / hry;
            float e = ex * ex + ey * ey;
            if (e < 1) {                           // 顶部白光：向白提亮
                float hl = 0.22f * (1 - e) * (1 - e);
                r += (255 - r) * hl; g += (255 - g) * hl; b += (255 - b) * hl;
            }
            float aP = a * fmin_(0.7f - sd, 1.0f);  // 圆角边缘软化
            if (aP < 0) aP = 0;
            g_bits[(u64)y * W + x] =
                (((u32)(aP * 255)) << 24) |
    ...
    // ---- 以下 GDI+ 件全部 clip 在面板圆角内 ----
    GpPath* panel = rr_path(L, T, R - 1, B - 1, RAD_SHELL);
    if (panel) GdipSetClipPath(g_gfx, panel, 0);

    // 折射光带：上部 1/3 处，宽 ~S(60)，sin 羽化白（峰值 0.13）
    if (g_bandTex) {
        int bandH = S(60);
        int bandY = T + (int)(ph * 0.33f);
        GdipDrawImageRectI(g_gfx, (GpImage*)g_bandTex, L, bandY - bandH / 2,
                           (int)pw, bandH);
    }
    // 镜面 streak ×2（左上 -> 右下对角，宽 80/40，alpha .08/.05）
    {
        float dl = xsqrt(pw * pw + ph * ph);
        float ddx = pw / dl, ddy = ph / dl;
        float c1x = L + pw * 0.55f, c1y = T + ph * 0.25f, hl1 = dl * 0.55f;
        streak(c1x - ddx * hl1, c1y - ddy * hl1, c1x + ddx * hl1, c1y + ddy * hl1,
               (float)S(80), 26);
        float c2x = L + pw * 0.34f, c2y = T + ph * 0.10f, hl2 = dl * 0.42f;
        streak(c2x - ddx * hl2, c2y - ddy * hl2, c2x + ddx * hl2, c2y + ddy * hl2,
               (float)S(40), 16);
    }
    GdipResetClip(g_gfx);
    if (panel) GdipDeletePath(panel);

    // 三层色散描边：外暖(左缘覆冷) / 中白 / 内暖(左右覆冷)——左冷右暖的棱镜方向性
    GpPen* pw70 = NULL, * pw40 = NULL, * pwh = NULL;
    GdipCreatePen1(C_WARM(70), 1.0f, GP_UNIT_PIXEL, &pw70);
    GdipCreatePen1(C_WARM(40), 1.0f, GP_UNIT_PIXEL, &pw40);
    GdipCreatePen1(ARGB(90, 255, 255, 255), 1.0f, GP_UNIT_PIXEL, &pwh);
    if (pw70) {
        stroke_rr(L, T, R - 1, B - 1, RAD_SHELL, pw70);
        GpPen* pc = NULL;
        if (GdipCreatePen1(C_ACCENT(70), 1.0f, GP_UNIT_PIXEL, &pc) == 0) {
            // 左缘覆冷（垂直直线段）
            GdipDrawLineI(g_gfx, pc, L, T + RAD_SHELL + 3, L, B - RAD_SHELL - 3);
            GdipDeletePen(pc);
        }
        GdipDeletePen(pw70);
    }
    if (pwh) {
        stroke_rr(L + 1, T + 1, R - 2, B - 2, RAD_SHELL - 1, pwh);
        GdipDeletePen(pwh);
    }
    if (pw40) {
        stroke_rr(L + 2, T + 2, R - 3, B - 3, RAD_SHELL - 2, pw40);
        GpPen* pc = NULL;
        if (GdipCreatePen1(C_ACCENT(40), 1.0f, GP_UNIT_PIXEL, &pc) == 0) {
            // 内层左右覆冷（上下保持暖 = 对称交换）
            GdipDrawLineI(g_gfx, pc, L + 2, T + RAD_SHELL + 2, L + 2, B - RAD_SHELL - 2);
            GdipDrawLineI(g_gfx, pc, R - 3, T + RAD_SHELL + 2, R - 3, B - RAD_SHELL - 2);
            GdipDeletePen(pc);
        }
        GdipDeletePen(pw40);
    }
    // 顶部内侧柔高光线（液体表面反光）
    GpPen* hi = NULL;
    if (GdipCreatePen1(ARGB(46, 255, 255, 255), 1.0f, GP_UNIT_PIXEL, &hi) == 0) {
        GdipDrawLineI(g_gfx, hi, L + RAD_SHELL + 4, T + 3, R - RAD_SHELL - 4, T + 3);
        GdipDeletePen(hi);
    }
...
static void panel_layout(HWND hwnd)
{
    ...
    if (g_mgr) {
        ...
        // 速发模式：检索行 → 液态 dock（标签胶囊 + 最近格同框一条）→ 格子阵列
        g_cell = S(56); g_gap = S(8);
        g_dock.left = g_panel.left + padX;
        g_dock.right = g_panel.right - padX;
        g_dock.top = g_searchBox.bottom + S(12);
        g_dock.bottom = g_dock.top + g_cell + S(12);
        // 最近格右对齐 dock（dock 右端 = 最新）；放不下的置空（不画、不命中）
        for (int i = 0; i < 7; i++) {
            g_recent[i].right = g_dock.right - S(10) - (6 - i) * (g_cell + g_gap);
            g_recent[i].left = g_recent[i].right - g_cell;
            g_recent[i].top = g_dock.top + S(6);
            g_recent[i].bottom = g_recent[i].top + g_cell;
            if (g_recent[i].left < g_dock.left + S(8)) {
                for (int j = i; j < 7; j++) SetRectEmpty(&g_recent[j]);
                break;
            }
        }
        g_grid.left = g_panel.left + padX;
        g_grid.top = g_dock.bottom + S(14);
        g_grid.right = g_panel.right - padX;
        g_footer.bottom = g_panel.bottom - padB;
        g_footer.top = g_footer.bottom - S(18);
        g_grid.bottom = g_footer.top - S(8);
...
static void draw_search_box(void)
{
    RECT rc = g_searchBox;
    int rad = RAD_CELL;
    // 内凹暗玻璃底：比面板更暗的液体腔 + 底缘内透光
    GpSolidFill* bg = NULL;
    if (GdipCreateSolidFill(ARGB(88, 8, 12, 22), &bg) == 0) {
        fill_rr(rc.left, rc.top, rc.right, rc.bottom, rad, (GpBrush*)bg);
        GdipDeleteBrush((GpBrush*)bg);
    }
    GpPen* bl = NULL;
    if (GdipCreatePen1(ARGB(26, 255, 255, 255), 1.0f, GP_UNIT_PIXEL, &bl) == 0) {
        GdipDrawLineI(g_gfx, bl, rc.left + rad + 2, rc.bottom - 2,
                      rc.right - rad - 2, rc.bottom - 2);
        GdipDeletePen(bl);
    }
    BOOL lit = g_search[0] || g_complen;
    GpPen* pen = NULL;
    u32 edge = lit ? C_ACCENT(120) : ARGB(30, 255, 255, 255);
    ...
    if (GdipCreatePen1(C_INK(128), 1.6f, GP_UNIT_PIXEL, &mp) == 0) {
        int cx = rc.left + S(15), cy = (rc.top + rc.bottom) / 2 - S(1);
        GdipDrawEllipseI(g_gfx, mp, cx - S(4), cy - S(5), S(8), S(8));
        GdipDrawLineI(g_gfx, mp, cx + S(2), cy + S(3), cx + S(6), cy + S(7));
        GdipDeletePen(mp);
    }
    ...
    // caret（空输入也显示，作为输入位置的引导）
    if (g_caretOn && !g_complen) {
        float preW = g_caret ? gtext_w(g_search, g_caret, g_fText) : 0.0f;
        int cxx = tx + (int)preW;
        GpPen* cp = NULL;
        if (GdipCreatePen1(C_ACCENT(255), 1.6f, GP_UNIT_PIXEL, &cp) == 0) {
    ...
    if (GdipCreateSolidFill(ARGB(chov ? 36 : 16, 255, 255, 255), &cbg) == 0) {
        fill_rr(cb.left, cb.top, cb.right, cb.bottom, S(9), (GpBrush*)cbg);
        GdipDeleteBrush((GpBrush*)cbg);
    }
    GpPen* xp = NULL;
    if (GdipCreatePen1(ARGB(chov ? 200 : 100, 231, 235, 246), 1.4f, GP_UNIT_PIXEL, &xp) == 0) {
        stroke_rr(cb.left, cb.top, cb.right, cb.bottom, S(9), xp);
        int m = S(9);
...
{
    // dock 最近格：液态玻璃折射底（格底贴图 + 提亮的边与顶线），右端 = 最新
    for (int i = 0; i < 7; i++) {
        int ei = (i < g_nRecent && g_recentIdx[i] < g_store.count) ? g_recentIdx[i] : -1;
        if (ei < 0) continue;
        RECT rc = g_recent[i];
        if (rc.right <= rc.left) continue;            // dock 放不下（布局已置空）
        BOOL hov = PtInRect(&rc, g_mouse);
        RECT dr = rc;
        if (hov) { dr.top -= S(2); dr.bottom -= S(2); }
        if (hov) draw_glow(g_glowSoft, rc, S(10));
        glass_cell(dr, RAD_CELL, hov ? 1 : 0, 0.030f);
        // 折射底：比普通格更亮的边 + 顶部细亮线（玻璃厚度感）
        GpPen* rp = NULL;
        if (GdipCreatePen1(ARGB(hov ? 150 : 46, 255, 255, 255), 1.0f, GP_UNIT_PIXEL, &rp) == 0) {
            stroke_rr(dr.left, dr.top, dr.right, dr.bottom, RAD_CELL, rp);
            GdipDrawLineI(g_gfx, rp, dr.left + RAD_CELL + 3, dr.top + 2,
                          dr.right - RAD_CELL - 3, dr.top + 2);
            GdipDeletePen(rp);
        }
        if (hov && g_anim.ei == ei && g_anim.pb) {
            draw_image_contain(g_anim.pb[g_anim.cur], g_anim.w, g_anim.h, dr, RAD_CELL);
            continue;
        }
        int iw, ih;
        GpBitmap* pb = oneshot_get(&g_recentImg[i], ei, &iw, &ih);
        if (pb) draw_image_contain(pb, iw, ih, dr, RAD_CELL);
    }
}

static void draw_chip(int* px, int y, const wchar_t* txt, int id)
{
    float w = gtext_w(txt, -1, g_fUi) + S(26);
    int h = S(26);
    RECT rc = { *px, y, *px + (int)w, y + h };
    // 右界：速发让开最近格，管理让开按钮组
    int bound;
    if (g_mgr) bound = g_closeRc.left - S(340);
    else if (g_nRecent > 0 && g_recent[0].right > g_recent[0].left)
        bound = g_recent[0].left - S(14);
    else bound = g_panel.right - S(16);
    if (rc.right > bound) return;    // 放不下的不画
    int active = (id == -1) ? (g_tagFilter == 0) : (g_tagFilter == ((u64)1 << id));
    int hov = PtInRect(&rc, g_mouse);

    if (active) draw_glow(g_glowStrong, rc, S(14));       // 激活胶囊外发光 peak .30
    glass_cell(rc, h / 2, active ? 2 : (hov ? 1 : 0), 0.045f);
    // 左上反光点（液面 specular）
    GpSolidFill* dot = NULL;
    if (GdipCreateSolidFill(active ? ARGB(220, 255, 255, 255) : ARGB(150, 255, 255, 255), &dot) == 0) {
        GdipFillEllipseI(g_gfx, (GpBrush*)dot, rc.left + S(11), rc.top + S(8), S(3), S(3));
        GdipDeleteBrush((GpBrush*)dot);
    }
    if (active)
        gtext_shadow(txt, -1, g_fUiB, rc, g_sfC, ARGB(250, 243, 248, 255));
    else
        gtext(txt, -1, g_fUi, rc, g_sfC, C_INK(hov ? 185 : 148));
...
// 液态 dock：一条内凹玻璃托盘，左端标签胶囊 + 分隔竖线 + 右端最近格（BRIEF：两行合一）
static void draw_dock(void)
{
    RECT rc = g_dock;
    GpSolidFill* tray = NULL;
    if (GdipCreateSolidFill(ARGB(60, 6, 9, 16), &tray) == 0) {
        fill_rr(rc.left, rc.top, rc.right, rc.bottom, RAD_CELL, (GpBrush*)tray);
        GdipDeleteBrush((GpBrush*)tray);
    }
    GpPen* tp = NULL;
    if (GdipCreatePen1(ARGB(24, 255, 255, 255), 1.0f, GP_UNIT_PIXEL, &tp) == 0) {
        stroke_rr(rc.left, rc.top, rc.right, rc.bottom, RAD_CELL, tp);
        // 托盘底缘内透光（液体表面底反光）
        GdipDrawLineI(g_gfx, tp, rc.left + RAD_CELL + 3, rc.bottom - 2,
                      rc.right - RAD_CELL - 3, rc.bottom - 2);
        GdipDeletePen(tp);
    }
    // 标签胶囊（垂直居中）
    g_nChips = 0;
    int chipH = S(26);
    int cx = rc.left + S(12);
    int cy = rc.top + (rc.bottom - rc.top - chipH) / 2;
    draw_chip(&cx, cy, L"全部", -1);
    for (int i = 0; i < MP_MAX_TAGS; i++)
        if (g_store.tagNames[i]) draw_chip(&cx, cy, g_store.tagNames[i], i);
    // 分隔竖线（最近格左侧）
    if (g_nRecent > 0 && g_recent[0].right > g_recent[0].left) {
        int dx = g_recent[0].left - S(11);
        GpSolidFill* dv = NULL;
        if (GdipCreateSolidFill(ARGB(46, 255, 255, 255), &dv) == 0) {
            fill_rr(dx, rc.top + S(16), dx + S(2), rc.bottom - S(16), S(1), (GpBrush*)dv);
            GdipDeleteBrush((GpBrush*)dv);
        }
    }
    draw_recents();
}
...
static void draw_empty(void)
{
    ...
    // 液态空态：一块虚影玻璃液滴 + 引导文字
    GpSolidFill* drop = NULL;
    if (GdipCreateSolidFill(ARGB(26, 255, 255, 255), &drop) == 0) {
        GdipFillEllipseI(g_gfx, (GpBrush*)drop,
                         cx - S(16), cy - S(38), S(32), S(32));
        GdipDeleteBrush((GpBrush*)drop);
    }
    GpPen* dp = NULL;
    if (GdipCreatePen1(ARGB(60, 255, 255, 255), 1.0f, GP_UNIT_PIXEL, &dp) == 0) {
        GdipDrawEllipseI(g_gfx, dp, cx - S(16), cy - S(38), S(32), S(32));
        GdipDeletePen(dp);
    }
    gtext(L"把表情图拖到这里", -1, g_fUi,
          (RECT){ g_grid.left, cy + S(2), g_grid.right, cy + S(22) }, g_sfC, C_INK(126));
}
...
static void draw_grid(void)
{
    ...
    for (int k = 0; k < g_cols * g_rows; k++) {
        ...
        if (hov) draw_glow(g_glowSoft, rc, S(10));
        glass_cell(dr, RAD_CELL, sel ? 2 : (hov ? 1 : 0), 0.030f);

        // 管理模式选中态描边（角标在图之后画，见下——contain 图会盖住先画的角标）
        if (sel) {
            GpPen* pen = NULL;
            if (GdipCreatePen1(C_ACCENT(210), 1.5f, GP_UNIT_PIXEL, &pen) == 0) {
                stroke_rr(dr.left, dr.top, dr.right, dr.bottom, RAD_CELL, pen);
                GdipDeletePen(pen);
            }
        }
        ...
        if (is_gif_name(e->name)) {
            ...
            GpSolidFill* bg = NULL;
            if (GdipCreateSolidFill(ARGB(178, 10, 12, 20), &bg) == 0) {
                fill_rr(gc.left, gc.top, gc.right, gc.bottom, S(4), (GpBrush*)bg);
                GdipDeleteBrush((GpBrush*)bg);
            }
            RECT gt = gc; gt.top -= S(1);
            gtext_shadow(L"GIF", -1, g_fTiny, gt, g_sfC, ARGB(230, 240, 247, 255));
        }
        // 管理模式选中角标：在图之上（contain 图会盖住角下的任何东西）
        if (sel) {
            RECT kc = { dr.left + S(5), dr.top + S(5), 0, dr.top + S(5) + S(16) };
            kc.right = kc.left + S(18);
            GpLineGradient* kb = NULL;
            GpPoint b1 = { kc.left, kc.bottom }, b2 = { kc.right, kc.top };
            if (GdipCreateLineBrushI(&b1, &b2, C_ACCENT(240), C_ACCENT2(228), GP_WRAP_TILE, &kb) == 0) {
                fill_rr_gradient(kc, S(5), kb);
        ...
        // hover 名字条（液态暗渐变 + 1px 下移暗影文字）
        if (hov) {
            RECT nb = { dr.left, dr.bottom - S(20), dr.right, dr.bottom };
            GpLineGradient* lg = NULL;
            GpPoint p1 = { 0, nb.top }, p2 = { 0, nb.bottom };
            if (GdipCreateLineBrushI(&p1, &p2, 0, ARGB(228, 6, 8, 15), GP_WRAP_TILE, &lg) == 0) {
                GpPath* clip = rr_path(dr.left, dr.top, dr.right, dr.bottom, RAD_CELL);
                if (clip) GdipSetClipPath(g_gfx, clip, 0);
            ...
            gtext_shadow(e->name, -1, g_fTiny, nt, g_sfC, ARGB(255, 248, 252, 255));
        }
    }
    // 细滚动条
    int vis = g_cols * g_rows;
    if (g_nFilt > vis) {
        ...
        GpSolidFill* bg = NULL;
        if (GdipCreateSolidFill(ARGB(60, 255, 255, 255), &bg) == 0) {
...
{
    // 平时留白；仅悬停格子时显示「文件名 · 标签」（功能性反馈，非常驻文案）
    ...
    gtext_shadow(line, -1, g_fSmall, lr, g_sfL, C_INK(158));
}
...
static void draw_preview(void)
{
    ...
    RECT rc = g_preview;
    // 大图：深色液体腔 + 色散微边（顶冷/底暖，呼应外壳棱镜）+ 内透光底缘
    GpSolidFill* bg = NULL;
    if (GdipCreateSolidFill(ARGB(238, 11, 13, 22), &bg) == 0) {
        fill_rr(rc.left, rc.top, rc.right, rc.bottom, RAD_CELL, (GpBrush*)bg);
        GdipDeleteBrush((GpBrush*)bg);
    }
    GpPen* pen = NULL;
    if (GdipCreatePen1(ARGB(78, 255, 255, 255), 1.2f, GP_UNIT_PIXEL, &pen) == 0) {
        stroke_rr(rc.left, rc.top, rc.right, rc.bottom, RAD_CELL, pen);
        GdipDeletePen(pen);
    }
    int ei = g_filt[g_previewIdx];
    if (g_anim.ei == ei && g_anim.pb) {
        draw_image_contain(g_anim.pb[g_anim.cur], g_anim.w, g_anim.h,
                           (RECT){ rc.left + 1, rc.top + 1, rc.right - 1, rc.bottom - 1 }, RAD_CELL - 1);
    } else {
        int iw, ih;
    ...
    // 色散微边在图之上：顶冷/底暖（呼应外壳棱镜方向性）
    GpPen* cw = NULL;
    if (GdipCreatePen1(C_ACCENT(150), 1.0f, GP_UNIT_PIXEL, &cw) == 0) {
        GdipDrawLineI(g_gfx, cw, rc.left + RAD_CELL + 3, rc.top + 2,
                      rc.right - RAD_CELL - 3, rc.top + 2);
        GdipDeletePen(cw);
    }
    GpPen* ww = NULL;
    if (GdipCreatePen1(C_WARM(130), 1.0f, GP_UNIT_PIXEL, &ww) == 0) {
        GdipDrawLineI(g_gfx, ww, rc.left + RAD_CELL + 3, rc.bottom - 2,
                      rc.right - RAD_CELL - 3, rc.bottom - 2);
        GdipDeletePen(ww);
    }
}

// 管理模式：操作行按钮（右端对齐排布，rect 记录命中）；液态胶囊
static void draw_mgr_button(int* pright, int y, const wchar_t* txt, int id, BOOL danger)
{
    ...
    int h = S(30);
    RECT rc = { *pright - (int)w, y, *pright, y + h };
    BOOL hov = PtInRect(&rc, g_mouse);
    BOOL on = (id == BTN_DELETE) ? g_delArm : FALSE;
    if (on) draw_glow(g_glowStrong, rc, S(12));
    glass_cell(rc, h / 2, on ? 2 : (hov ? 1 : 0), 0.03f);
    if (on) {   // 武装态：青->紫液态底由 glass_cell(2) 给，再描红边强调危险
        GpPen* pen = NULL;
        if (GdipCreatePen1(ARGB(230, 255, 110, 110), 1.2f, GP_UNIT_PIXEL, &pen) == 0) {
            stroke_rr(rc.left, rc.top, rc.right, rc.bottom, h / 2, pen);
            GdipDeletePen(pen);
        }
    } else if (danger && hov) {
        GpPen* pen = NULL;
        if (GdipCreatePen1(ARGB(150, 255, 120, 120), 1.0f, GP_UNIT_PIXEL, &pen) == 0) {
            stroke_rr(rc.left, rc.top, rc.right, rc.bottom, h / 2, pen);
            GdipDeletePen(pen);
        }
    }
    if (on)
        gtext_shadow(txt, -1, g_fUiB, rc, g_sfC, ARGB(255, 255, 240, 244));
    else
        gtext(txt, -1, g_fUi, rc, g_sfC, C_INK(hov ? 195 : 155));
    g_btnRc[id] = rc;
    *pright = rc.left - S(8);
}
...
static void panel_repaint(void)
{
    ...
    if (g_mgr) {
        ...
        draw_dock();     // 液态 dock：标签胶囊 + 最近格同框一条
        draw_grid();
        draw_footer();
    }
    draw_preview();
...
static void ime_update_position(HWND hwnd)
{
    ...
    if (!im) return;
    float preW = g_caret ? gtext_w(g_search, g_caret, g_fText) : 0.0f;
    int cxx = g_searchBox.left + S(30) + (int)preW;
...
static HICON make_app_icon(void)
{
    ...
    if (color && mask && colorBits && maskBits) {
        ...
        // 深玻璃底：对角渐变 液态蓝 (96,118,164) -> 深底 (25,30,45)，手写像素
        u32* px = (u32*)colorBits;
        for (int y = 0; y < 64; y++)
            for (int x = 0; x < 64; x++) {
                float t = (x + y) / 126.0f;
                u8 r = (u8)(96 + (25 - 96) * t), g = (u8)(118 + (30 - 118) * t), b = (u8)(164 + (45 - 164) * t);
                px[y * 64 + x] = 0xFF000000u | (r << 16) | (g << 8) | b;   // alpha 修补：icon 走 ARGB 路径也正确
            }
        HDC mdc = CreateCompatibleDC(sdc);
        HGDIOBJ old = SelectObject(mdc, color);
        // 内部反渐变覆盖出圆角（用深色圆角矩形裁形）
        HBRUSH glass = CreateSolidBrush(RGB(24, 29, 44));
        HBRUSH glass2 = CreateSolidBrush(RGB(58, 78, 122));
        HPEN edge = CreatePen(PS_SOLID, 3, RGB(111, 211, 255));
...
void entry(void)
{
    ...
    g_fUiB   = make_font(fdc, S(12), FW_SEMIBOLD, L"Segoe UI");
    ...
    make_glow(&g_glowStrong, &g_glowStrongBuf, 0.30f);   // 激活胶囊外发光 peak .30
    make_cell_tex(&g_cellTex,     &g_cellTexBuf,     0.10f, 0.02f);  // 格底 normal
    make_cell_tex(&g_cellTexHot,  &g_cellTexHotBuf,  0.20f, 0.05f);  // 格底 hover（更亮一档）
    make_band_tex(&g_bandTex, &g_bandTexBuf, 0.13f);                 // 折射光带 peak .13
