...
// v0.10：任意支持格式 → 首帧全尺寸 straight BGRA（顶朝下；HeapAlloc，调用方 HeapFree）。
// 剪贴板 CF_DIB 静态兜底的像素源（不能用 256px 缩略图，要原图）。
BOOL wic_load_bgra(const wchar_t* path, int* ow, int* oh, u8** out)
{
    *out = NULL; *ow = *oh = 0;
    if (!g_factory) return FALSE;
    IWICBitmapDecoder* dec = NULL;
    ...
    if (SUCCEEDED(IWICBitmapDecoder_GetFrame(dec, 0, &frame)) && frame) {
        UINT w = 0, h = 0;
        if (SUCCEEDED(IWICBitmapFrameDecode_GetSize(frame, &w, &h)) && w && h &&
            w <= 8192 && h <= 8192 && (u64)w * h * 4 < 96ull * 1024 * 1024 &&
            SUCCEEDED(IWICImagingFactory_CreateFormatConverter(g_factory, &conv)) && conv &&
            SUCCEEDED(IWICFormatConverter_Initialize(
                ...
                WICBitmapDitherTypeNone, NULL, 0.0, WICBitmapPaletteTypeCustom)))
        {
            ...
            if (buf) {
                if (SUCCEEDED(IWICFormatConverter_CopyPixels(conv, NULL, w * 4, w * h * 4, buf))) {
                    *out = buf; *ow = (int)w; *oh = (int)h; ok = TRUE;
                } else {
