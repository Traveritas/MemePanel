...
// ---- 「宣纸墨 PAPER-INK」tokens（BRIEF.md）----
// 浓墨 #33302B；次墨 α158；弱墨 α97。印章红（朱砂印泥）#C3402B 是唯一强调色：
// 只出现在 激活胶囊 / 聚焦搜索线 / hover 边框 / 武装删除 / 选中角标（+两粒 2-3px 句读小红点）。
#define T_INK_R 51                 // 浓墨 #33302B
#define T_INK_G 48
#define T_INK_B 43
#define T_ACC_R 195                // 印章红 #C3402B
#define T_ACC_G 64
#define T_ACC_B 43
...
#define C_INK(a)    ARGB(a, T_INK_R, T_INK_G, T_INK_B)
#define C_ACCENT(a) ARGB(a, T_ACC_R, T_ACC_G, T_ACC_B)
#define C_PAPER_TXT(a) ARGB(a, 255, 253, 248)   // 纸上白字（暖白，用于红/墨底之上）
...
// 字体 / 字符串格式 / 笔刷
static GpFont* g_fText, * g_fUi, * g_fSmall, * g_fTiny, * g_fMono, * g_fGlyph;
static GpStringFormat *g_sfL, * g_sfC, * g_sfR, * g_sfCF;
static GpPen* g_penEdge;      // 墨色 .18（宣纸面板边框）
...
static BOOL g_focus = TRUE;   // 面板持有键盘焦点（搜索底线：红=聚焦 / 墨=失焦）；无头渲染默认聚焦
...
// 纸纤维噪点的确定性随机（xorshift32；仅底图生成期使用，不进每帧路径）
static u32 g_rng = 0x6D2B79F5u;
...
// 宣纸格子三态（paper-ink tokens）：
//   normal = 微白卡纸底（略比面板白）+ 墨色发丝边
//   hover  = 印章红 α0.06 淡底 + 红边 α120（唯一允许红边的场合）
//   active = 印章红实底（武装删除 / 需要强调的激活件）
static void paper_cell(RECT rc, int rad, int state, float baseA)
{
    GpSolidFill* bg = NULL;
    u32 col = state == 2 ? C_ACCENT(238)
             : state == 1 ? ARGB(16, T_ACC_R, T_ACC_G, T_ACC_B)
    ...
    GpPen* pen = NULL;
    u32 pc = state == 2 ? C_ACCENT(255)
            : state == 1 ? C_ACCENT(120)
            : ARGB(30, 60, 50, 40);
    if (GdipCreatePen1(pc, state == 1 ? 1.2f : 1.0f, GP_UNIT_PIXEL, &pen) == 0) {
...
// 底图：暖白宣纸垂直渐变 rgba(248,246,240,.93) → rgba(238,235,226,.90)，
// 无径向高光；改为纸纤维噪点（约 2% 像素 ±3 亮度，生成期随机一次）。
// 边 = 1px 墨色 .18 + inset 白顶线 .8（GDI+ AA）。四周纯透明（无投影，出血仅圆角 AA 余量）。
static void gen_base(void)
{
    ...
    float rad = (float)S(14);                 // 外壳圆角 14（比琉璃版 18 收敛）
    ...
    for (int y = 0; y < H; y++) {
        ...
        for (int x = 0; x < W; x++) {
            ...
            float t = (fy - cy) / ph + 0.5f;           // 垂直渐变：顶暖白 → 底微沉
            if (t < 0) t = 0; if (t > 1) t = 1;
            float r = 248 + (238 - 248) * t, g = 246 + (235 - 246) * t, b = 240 + (226 - 240) * t;
            if ((rng_u32() & 1023) < 21) {             // ~2% 纸纤维
                int d = (int)(rng_u32() % 7) - 3;      // ±3 亮度（同加三通道，只变明度不变色相）
                r += d; g += d; b += d;
            }
            float aP = (0.93f + (0.90f - 0.93f) * t) * fmin_(0.7f - sd, 1.0f);  // 圆角边缘软化
            if (aP < 0) aP = 0;
            g_bits[(u64)y * W + x] =
                (((u32)(aP * 255)) << 24) |
    ...
    // 墨色 1px 边 + inset 白顶线（GDI+，AA）
    GpPath* edge = rr_path(L, T, R - 1, B - 1, S(14));
    if (edge) { GdipDrawPath(g_gfx, g_penEdge, edge); GdipDeletePath(edge); }
    GpPen* hi = NULL;
    if (GdipCreatePen1(ARGB(204, 255, 255, 255), 1.0f, GP_UNIT_PIXEL, &hi) == 0) {
        GdipDrawLineI(g_gfx, hi, L + S(14) + 2, T + 2, R - S(14) - 2, T + 2);
        GdipDeletePen(hi);
    }
...
// ============================================================
// 布局（宣纸墨结构：检索行 / 最近7格（册页装订线）/ 墨书签标签行 / 网格 / 信息行）
// ============================================================
...
static void draw_search_box(void)
{
    ...
    // 「搜」字墨记（楷体，次墨色）——放大镜的文人替身
    RECT gr = { rc.left, rc.top, rc.left + S(28), rc.bottom };
    gtext(L"搜", 1, g_fGlyph, gr, g_sfC, C_INK(155));

    // 无框：只一条底部墨线。面板持焦（搜索即主交互）时转印章红并加宽——
    // 这是「输入落笔处」的唯一标记
    GpPen* lp = NULL;
    if (GdipCreatePen1(g_focus ? C_ACCENT(235) : C_INK(110),
                       g_focus ? 2.0f : 1.5f, GP_UNIT_PIXEL, &lp) == 0) {
        GdipDrawLineI(g_gfx, lp, rc.left + 1, rc.bottom - 1, rc.right - 1, rc.bottom - 1);
        GdipDeletePen(lp);
    }
    ...
    if (g_search[0] || g_complen) {
        ...
        if (g_complen) {
            ...
            GpPen* up = NULL;
            if (GdipCreatePen1(C_INK(175), 1.4f, GP_UNIT_PIXEL, &up) == 0) {
    ...
    // caret：墨色竖线 2px（空输入也显示，作为输入位置的引导；红只留给底线）
    if (g_caretOn && !g_complen) {
        float preW = g_caret ? gtext_w(g_search, g_caret, g_fText) : 0.0f;
        int cxx = tx + (int)preW;
        GpPen* cp = NULL;
        if (GdipCreatePen1(C_INK(228), 2.0f, GP_UNIT_PIXEL, &cp) == 0) {
    ...
    // 关闭按钮：空心墨圈 ○（1.5px 墨笔圆）；hover 圈心落一粒朱砂
    RECT cb = g_closeRc;
    BOOL chov = PtInRect(&cb, g_mouse);
    int ccx = (cb.left + cb.right) / 2, ccy = (cb.top + cb.bottom) / 2;
    int crr = (cb.right - cb.left) / 2 - S(3);
    GpPen* op = NULL;
    if (GdipCreatePen1(C_INK(chov ? 215 : 145), 1.5f, GP_UNIT_PIXEL, &op) == 0) {
        GdipDrawEllipseI(g_gfx, op, ccx - crr, ccy - crr, crr * 2, crr * 2);
        GdipDeletePen(op);
    }
    if (chov) {
        GpSolidFill* dot = NULL;
        if (GdipCreateSolidFill(C_ACCENT(215), &dot) == 0) {
            int dsz = S(6);
            fill_rr(ccx - dsz / 2, ccy - dsz / 2, ccx + dsz / 2, ccy + dsz / 2, dsz / 2, (GpBrush*)dot);
...
{
    // 最近快捷行（册页）：与网格完全同款格子；无内容不画占位，不带数字角标。
    // 整行左侧一条 2px 墨色装订线，行末一粒小红点作句读分隔。
    if (g_nRecent > 0) {
        GpPen* bind = NULL;
        if (GdipCreatePen1(ARGB(40, T_INK_R, T_INK_G, T_INK_B), 2.0f, GP_UNIT_PIXEL, &bind) == 0) {
            GdipDrawLineI(g_gfx, bind,
                          g_recent[0].left - S(9),
                          g_recent[0].top + S(2),
                          g_recent[0].left - S(9),
                          g_recent[0].bottom - S(2));
            GdipDeletePen(bind);
        }
    }
    int lastDrawn = -1;
    for (int i = 0; i < 7; i++) {
        ...
        paper_cell(dr, S(8), hov ? 1 : 0, 0.16f);
        if (hov && g_anim.ei == ei && g_anim.pb) {
            draw_image_contain(g_anim.pb[g_anim.cur], g_anim.w, g_anim.h, dr, S(8));
            lastDrawn = i;
            continue;
        }
        int iw, ih;
        GpBitmap* pb = oneshot_get(&g_recentImg[i], ei, &iw, &ih);
        if (pb) draw_image_contain(pb, iw, ih, dr, S(8));
        lastDrawn = i;
    }
    // 行末小红点（句读）
    if (lastDrawn >= 0) {
        GpSolidFill* dot = NULL;
        if (GdipCreateSolidFill(C_ACCENT(190), &dot) == 0) {
            int dx = g_recent[lastDrawn].right + S(6);
            int dy = (g_recent[lastDrawn].top + g_recent[lastDrawn].bottom) / 2;
            int dr2 = S(2);
            fill_rr(dx - dr2, dy - dr2, dx + dr2, dy + dr2, dr2, (GpBrush*)dot);
            GdipDeleteBrush((GpBrush*)dot);
        }
    }
}

// 墨书签：常态 = 无底纯文字（次墨色，hover 转浓）；激活 = 印章式：朱砂底白字
// 圆角 4px + 右上 1px 暗红 45° 凹痕短线（仿印章边缘）
static void draw_chip(int* px, int y, const wchar_t* txt, int id)
{
    ...
    if (active) {
        GpSolidFill* seal = NULL;
        if (GdipCreateSolidFill(C_ACCENT(238), &seal) == 0) {
            fill_rr(rc.left, rc.top, rc.right, rc.bottom, S(4), (GpBrush*)seal);
            GdipDeleteBrush((GpBrush*)seal);
        }
        gtext(txt, -1, g_fUi, rc, g_sfC, C_PAPER_TXT(255));
        // 印章边缘凹痕：右上 45° 暗红短线
        GpPen* np = NULL;
        if (GdipCreatePen1(ARGB(90, 122, 24, 14), 1.0f, GP_UNIT_PIXEL, &np) == 0) {
            GdipDrawLineI(g_gfx, np, rc.right - S(7), rc.top + S(2), rc.right - S(2), rc.top + S(7));
            GdipDeletePen(np);
        }
    } else {
        gtext(txt, -1, g_fUi, rc, g_sfC, C_INK(hov ? 222 : 165));
    }
...
static void draw_empty(void)
{
    ...
    gtext(L"拖入表情，落纸为安", -1, g_fUi,
          (RECT){ g_grid.left, cy - S(14), g_grid.right, cy + S(6) }, g_sfC, C_INK(150));
    GpPen* ln = NULL;
    if (GdipCreatePen1(C_INK(45), 1.0f, GP_UNIT_PIXEL, &ln) == 0) {
        GdipDrawLineI(g_gfx, ln, cx - S(56), cy + S(16), cx + S(56), cy + S(16));
        GdipDeletePen(ln);
    }
}
...
static void draw_grid(void)
{
    ...
    for (int k = 0; k < g_cols * g_rows; k++) {
        ...
        // 管理模式选中态：印章红描边 + 左上双勾角标（红纸上两个小勾）
        if (sel) {
            GpPen* pen = NULL;
            if (GdipCreatePen1(C_ACCENT(215), 1.6f, GP_UNIT_PIXEL, &pen) == 0) {
                stroke_rr(dr.left, dr.top, dr.right, dr.bottom, S(8), pen);
                GdipDeletePen(pen);
            }
            RECT kc = { dr.left + S(5), dr.top + S(5), dr.left + S(5) + S(18), dr.top + S(5) + S(15) };
            GpSolidFill* kb = NULL;
            if (GdipCreateSolidFill(C_ACCENT(235), &kb) == 0) {
                fill_rr(kc.left, kc.top, kc.right, kc.bottom, S(4), (GpBrush*)kb);
                GdipDeleteBrush((GpBrush*)kb);
            }
            GpPen* kp = NULL;
            if (GdipCreatePen1(C_PAPER_TXT(255), 1.7f, GP_UNIT_PIXEL, &kp) == 0) {
                int ky1 = kc.top + S(9), ky2 = kc.top + S(11);
                GdipDrawLineI(g_gfx, kp, kc.left + S(2), ky1, kc.left + S(4), ky2);
                GdipDrawLineI(g_gfx, kp, kc.left + S(4), ky2, kc.left + S(7), kc.top + S(4));
                GdipDrawLineI(g_gfx, kp, kc.left + S(8), ky1, kc.left + S(10), ky2);
                GdipDrawLineI(g_gfx, kp, kc.left + S(10), ky2, kc.left + S(13), kc.top + S(4));
                GdipDeletePen(kp);
            }
        }
        ...
        if (is_gif_name(e->name)) {
            ...
            GpSolidFill* bg = NULL;
            if (GdipCreateSolidFill(ARGB(200, T_INK_R, T_INK_G, T_INK_B), &bg) == 0) {
                fill_rr(gc.left, gc.top, gc.right, gc.bottom, S(4), (GpBrush*)bg);
                GdipDeleteBrush((GpBrush*)bg);
            }
            RECT gt = gc; gt.top -= S(1);
            gtext(L"GIF", -1, g_fTiny, gt, g_sfC, C_PAPER_TXT(235));
        }
        // hover 名字条：墨色渐变遮罩（#33302B，非纯黑）+ 暖白小字
        if (hov) {
            RECT nb = { dr.left, dr.bottom - S(20), dr.right, dr.bottom };
            GpLineGradient* lg = NULL;
            GpPoint p1 = { 0, nb.top }, p2 = { 0, nb.bottom };
            if (GdipCreateLineBrushI(&p1, &p2, 0, ARGB(215, T_INK_R, T_INK_G, T_INK_B), GP_WRAP_CLAMP, &lg) == 0) {
                GpPath* clip = rr_path(dr.left, dr.top, dr.right, dr.bottom, S(8));
                if (clip) GdipSetClipPath(g_gfx, clip, 0);
            ...
            gtext(e->name, -1, g_fTiny, nt, g_sfC, C_PAPER_TXT(255));
        }
    }
    // 细滚动条（墨）
    int vis = g_cols * g_rows;
    if (g_nFilt > vis) {
        ...
        GpSolidFill* bg = NULL;
        if (GdipCreateSolidFill(ARGB(80, T_INK_R, T_INK_G, T_INK_B), &bg) == 0) {
...
static void draw_footer(void)
{
    ...
    gtext(line, -1, g_fSmall, lr, g_sfL, C_INK(158));   // 次墨 · 宋体
}
...
static void draw_preview(void)
{
    ...
    RECT rc = g_preview;
    // 裱卡：比面板更白的纸卡 + 墨色发丝边 + 圆角 8（无投影，靠纸的明度分层）
    GpSolidFill* bg = NULL;
    if (GdipCreateSolidFill(ARGB(242, 255, 253, 247), &bg) == 0) {
        fill_rr(rc.left, rc.top, rc.right, rc.bottom, S(8), (GpBrush*)bg);
        GdipDeleteBrush((GpBrush*)bg);
    }
    GpPen* pen = NULL;
    if (GdipCreatePen1(ARGB(60, 60, 50, 40), 1.2f, GP_UNIT_PIXEL, &pen) == 0) {
        stroke_rr(rc.left, rc.top, rc.right, rc.bottom, S(8), pen);
        GdipDeletePen(pen);
    }
    int ei = g_filt[g_previewIdx];
    if (g_anim.ei == ei && g_anim.pb) {
        draw_image_contain(g_anim.pb[g_anim.cur], g_anim.w, g_anim.h,
                           (RECT){ rc.left + 1, rc.top + 1, rc.right - 1, rc.bottom - 1 }, S(7));
        return;
    }
    int iw, ih;
...
// 管理模式按钮（墨线描边书签式）：常态 = 无底 + 墨描边 + 次墨字；hover = 墨底白字；
// 删除武装 = 印章红实底。固定按武装文案宽度记录命中（武装态不改 rect，二次确认不点空）。
static void draw_mgr_button(int* pright, int y, const wchar_t* txt, int id, BOOL danger)
{
    ...
    if (on) {
        // 武装删除：印章红实底
        GpSolidFill* b = NULL;
        if (GdipCreateSolidFill(C_ACCENT(238), &b) == 0) {
            fill_rr(rc.left, rc.top, rc.right, rc.bottom, S(4), (GpBrush*)b);
            GdipDeleteBrush((GpBrush*)b);
        }
        gtext(txt, -1, g_fUi, rc, g_sfC, C_PAPER_TXT(255));
    } else if (hov) {
        // 墨底白字
        GpSolidFill* b = NULL;
        if (GdipCreateSolidFill(C_INK(232), &b) == 0) {
            fill_rr(rc.left, rc.top, rc.right, rc.bottom, S(4), (GpBrush*)b);
            GdipDeleteBrush((GpBrush*)b);
        }
        gtext(txt, -1, g_fUi, rc, g_sfC, C_PAPER_TXT(255));
        ...
        // 墨线描边书签
        GpPen* pen = NULL;
        if (GdipCreatePen1(C_INK(130), 1.2f, GP_UNIT_PIXEL, &pen) == 0) {
            stroke_rr(rc.left, rc.top, rc.right, rc.bottom, S(4), pen);
            GdipDeletePen(pen);
        }
        gtext(txt, -1, g_fUi, rc, g_sfC, C_INK(170));
    }
    g_btnRc[id] = rc;
    *pright = rc.left - S(8);
    (void)danger;
...
static HICON make_app_icon(void)
{
    ...
    if (color && mask && colorBits && maskBits) {
        ...
        // 底：暖墨对角渐变（露在圆角外的四角），中心覆盖宣纸圆角方
        u32* px = (u32*)colorBits;
        for (int y = 0; y < 64; y++)
            for (int x = 0; x < 64; x++) {
                float t = (x + y) / 126.0f;
                u8 r = (u8)(58 + (40 - 58) * t), g = (u8)(54 + (37 - 54) * t), b = (u8)(48 + (33 - 48) * t);
                px[y * 64 + x] = 0xFF000000u | (r << 16) | (g << 8) | b;   // alpha 修补：icon 走 ARGB 路径也正确
            }
        HDC mdc = CreateCompatibleDC(sdc);
        HGDIOBJ old = SelectObject(mdc, color);
        // 宣纸圆角方：微沉纸底 + 墨边
        HBRUSH paper  = CreateSolidBrush(RGB(248, 246, 240));
        HBRUSH paper2 = CreateSolidBrush(RGB(238, 235, 226));
        HPEN edge = CreatePen(PS_SOLID, 3, RGB(60, 50, 40));
        rr_gdi(mdc, 7, 7, 57, 57, 16, paper2, edge);
        rr_gdi(mdc, 11, 11, 53, 53, 13, paper, NULL);
        // 墨笑脸
        HPEN ip = CreatePen(PS_SOLID, 4, RGB(51, 48, 43));
        HBRUSH ib = CreateSolidBrush(RGB(51, 48, 43));
        HGDIOBJ op = SelectObject(mdc, ip);
        HGDIOBJ ob = SelectObject(mdc, ib);
        int e = 2;
        ...
        SelectObject(mdc, op);
        // 朱砂小印（右下角）
        HBRUSH seal = CreateSolidBrush(RGB(195, 64, 43));
        rr_gdi(mdc, 45, 45, 58, 58, 3, seal, NULL);
        SelectObject(mdc, old);
        DeleteDC(mdc);
        DeleteObject(paper); DeleteObject(paper2); DeleteObject(edge);
        DeleteObject(ib); DeleteObject(ip); DeleteObject(seal);
...
void entry(void)
{
    ...
    // 字体 / 格式 / 质感件
    // 字体（宣纸墨：楷体题签 / 宋体正文 / Consolas 计数）
    HDC fdc = GetDC(NULL);
    g_fText  = make_font(fdc, S(13), FW_NORMAL,   L"SimSun");    // 正文/搜索输入：宋体
    g_fUi    = make_font(fdc, S(13), FW_NORMAL,   L"KaiTi");     // 标签/按钮：楷体（大 1px 显笔势）
    g_fSmall = make_font(fdc, S(11), FW_NORMAL,   L"SimSun");    // 文件名/状态行
    g_fTiny  = make_font(fdc, S(9),  FW_NORMAL,   L"SimSun");
    g_fMono  = make_font(fdc, S(10), FW_NORMAL,   L"Consolas");
    g_fGlyph = make_font(fdc, S(12), FW_NORMAL,   L"KaiTi");     // 「搜」字墨记
    ReleaseDC(NULL, fdc);
    ...
    GdipCreatePen1(ARGB(46, 60, 50, 40), 1.0f, GP_UNIT_PIXEL, &g_penEdge);   // 墨色 1px 边（α.18）
