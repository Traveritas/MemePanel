...
{
    if (state == 2) {   // 激活（胶囊用）：120° accent->accent2 渐变
        GpLineGradient* lg = NULL;
        GpPoint p1 = { rc.left, rc.bottom - S(4) }, p2 = { rc.right, rc.top + S(4) };
        if (GdipCreateLineBrushI(&p1, &p2, C_ACCENT(77), C_ACCENT2(56), GP_WRAP_CLAMP, &lg) == 0) {
...
static void draw_empty(void)
{
    ...
    gtext(L"把表情图拖到这里", -1, g_fUi,
          (RECT){ g_grid.left, cy - S(12), g_grid.right, cy + S(8) }, g_sfC, C_INK(120));
}
...
static void draw_grid(void)
{
    ...
    for (int k = 0; k < g_cols * g_rows; k++) {
        ...
        if (hov) {
            ...
            if (GdipCreateLineBrushI(&p1, &p2, 0, ARGB(225, 8, 9, 14), GP_WRAP_CLAMP, &lg) == 0) {
...
static void draw_footer(void)
{
    ...
    for (int t = 0; t < MP_MAX_TAGS && n < 155; t++) {
        ...
        if (!first) { line[n++] = L' '; line[n++] = L'/'; line[n++] = L' '; }
        first = 0;
        const wchar_t* tn = g_store.tagNames[t];
        while (*tn && n < 155) line[n++] = *tn++;
    }
    line[n] = 0;
