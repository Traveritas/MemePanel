...
// ---- 「琉璃 HALO」tokens（halo-spec.md §2）----
#define T_INK    RGB(231,235,246)   // 主文字
#define T_ACC_R  122                // ACCENT  #7AA2FF
#define T_ACC_G  162
#define T_ACC_B  255
#define T_ACC2_R 178                // ACCENT2  #B28BFF
#define T_ACC2_G 139
#define T_ACC2_B 255

// ---- refine 轮工艺 token（BRIEF 01-halo-refine）----
#define HOV_EDGE_A  230    // hover 亮边 alpha：191 -> 230（缩到 50% 仍可见）
#define HOV_LIFT    3      // hover 上浮逻辑 px：2 -> 3
#define HOV_GLOW    0.34f  // hover 微光 peak：0.22 -> 0.34
#define FOCUS_GLOW  0.10f  // 搜索框聚焦外发光 peak
#define DANGER1_R 255      // 删除武装态渐变 #FF6E6E -> #E34545
#define DANGER1_G 110
#define DANGER1_B 110
#define DANGER2_R 227
#define DANGER2_G 69
#define DANGER2_B 69
...
// 质感件（一次生成）
static GpBitmap* g_glowSoft, * g_glowStrong, * g_glowFocus;
static u8*   g_glowSoftBuf, * g_glowStrongBuf, * g_glowFocusBuf;

// 字体 / 字符串格式 / 笔刷
static GpFont* g_fText, * g_fUi, * g_fSmall, * g_fTiny, * g_fName, * g_fMono;
static GpFontFamily* g_fFam;   // 9.5px 半磅字需 REAL emSize（LOGFONT 整数高度表达不了）
static GpStringFormat *g_sfL, * g_sfC, * g_sfR, * g_sfCF, * g_sfNW;   // g_sfNW：居中 + NoWrap（名字条单行省略）
...
// 渐变胶囊底（120° 对角）
static void pill_gradient(RECT rc, int rad, u32 cA, u32 cB)
{
    GpLineGradient* lg = NULL;
    GpPoint p1 = { rc.left, rc.bottom - S(4) }, p2 = { rc.right, rc.top + S(4) };
    if (GdipCreateLineBrushI(&p1, &p2, cA, cB, GP_WRAP_CLAMP, &lg) == 0) {
        fill_rr(rc.left, rc.top, rc.right, rc.bottom, rad, (GpBrush*)lg);
        GdipDeleteBrush((GpBrush*)lg);
    }
}

// 胶囊厚度感：顶部 1px 内亮线 + 下缘 1px 接触阴影（45% 暗）
static void pill_thickness(RECT rc, int rad)
{
    GpPen* pl = NULL;
    if (GdipCreatePen1(ARGB(92, 255, 255, 255), 1.0f, GP_UNIT_PIXEL, &pl) == 0) {
        GdipDrawLineI(g_gfx, pl, rc.left + rad + 2, rc.top + 2, rc.right - rad - 2, rc.top + 2);
        GdipDeletePen(pl);
    }
    GpPen* ps = NULL;
    if (GdipCreatePen1(ARGB(115, 5, 7, 12), 1.0f, GP_UNIT_PIXEL, &ps) == 0) {
        GdipDrawLineI(g_gfx, ps, rc.left + rad + 2, rc.bottom - 2, rc.right - rad - 2, rc.bottom - 2);
        GdipDeletePen(ps);
    }
}

// 玻璃格底 + 边 + inset 顶亮线（normal/hover/active 三态，halo tokens）
static void glass_cell(RECT rc, int rad, int state, float baseA)
{
    if (state == 2) {   // 激活（胶囊用）：120° accent->accent2 渐变 + 厚度感
        pill_gradient(rc, rad, C_ACCENT(77), C_ACCENT2(56));
    } else {
        GpSolidFill* bg = NULL;
    ...
    u32 pc = state == 2 ? C_ACCENT(153)
            : state == 1 ? C_ACCENT(HOV_EDGE_A) : ARGB((int)(baseA > 0.04f ? 31 : 19), 255, 255, 255);
    if (GdipCreatePen1(pc, 1.0f, GP_UNIT_PIXEL, &pen) == 0) {
        stroke_rr(rc.left, rc.top, rc.right, rc.bottom, rad, pen);
        GdipDeletePen(pen);
    }
    if (state == 2) {   // 激活胶囊：厚度感双线（替代普通 inset 顶线）
        pill_thickness(rc, rad);
        return;
    }
    // inset 顶亮线
    if (rad * 2 < rc.right - rc.left) {
        GpPen* pl = NULL;
        u32 lc = state ? ARGB((int)(0.20f * 255), 255, 255, 255) : ARGB((int)(0.06f * 255), 255, 255, 255);
...
// 删除武装态：红色系专属渐变 #FF6E6E -> #E34545（语义色与 accent 分离）+ 厚度感
static void danger_pill(RECT rc, int rad)
{
    pill_gradient(rc, rad, ARGB(238, DANGER1_R, DANGER1_G, DANGER1_B),
                        ARGB(238, DANGER2_R, DANGER2_G, DANGER2_B));
    GpPen* pen = NULL;
    if (GdipCreatePen1(ARGB(235, 255, 128, 128), 1.0f, GP_UNIT_PIXEL, &pen) == 0) {
        stroke_rr(rc.left, rc.top, rc.right, rc.bottom, rad, pen);
        GdipDeletePen(pen);
    }
    pill_thickness(rc, rad);
...
static void free_statics(void)
{
    ...
    if (g_glowFocus) { GdipDisposeImage((GpImage*)g_glowFocus); g_glowFocus = NULL; }
    ...
    if (g_glowFocusBuf) { HeapFree(GetProcessHeap(), 0, g_glowFocusBuf); g_glowFocusBuf = NULL; }
}

// 底图：164° 线性渐变 + 顶部径向高光 + GDI+ AA 边框（用户决定：无投影阴影，四周纯透明）
static void gen_base(void)
{
    ...
    float rad = (float)S(18);
    // 164° 渐变轴（CSS 角度 -> 屏幕方向 (sin, -cos)）
    float dirx = 0.27564f, diry = 0.96126f;
    float hw = pw / 2, hh = ph / 2;
    float ext = hw * dirx + hh * diry + 0.001f;
    // 顶部径向高光（620x190 @ 10% -6%，按面板宽等比）
    float hcx = L + pw * 0.10f, hcy = T - ph * 0.06f;
    float hrx = pw * 620.0f / 700.0f, hry = pw * 190.0f / 700.0f;
    ...
    for (int y = 0; y < H; y++) {
        ...
        for (int x = 0; x < W; x++) {
            ...
            float t = ((fx - cx) * dirx + (fy - cy) * diry) / ext + 0.5f;
            if (t < 0) t = 0; if (t > 1) t = 1;
            float r = 28 + (17 - 28) * t, g = 31 + (19 - 31) * t, b = 44 + (29 - 44) * t;
            float ex = (fx - hcx) / hrx, ey = (fy - hcy) / hry;
            float e = ex * ex + ey * ey;
            if (e < 1) {                           // 顶部高光：向白提亮
                float hl = 0.10f * (1 - e) * (1 - e);
                r += (255 - r) * hl; g += (255 - g) * hl; b += (255 - b) * hl;
            }
            float aP = (0.94f + (0.90f - 0.94f) * t) * fmin_(0.7f - sd, 1.0f);  // 圆角边缘软化
            if (aP < 0) aP = 0;
            g_bits[(u64)y * W + x] =
                (((u32)(aP * 255)) << 24) |
    ...
    GpPath* edge = rr_path(L, T, R - 1, B - 1, S(18));
    if (edge) { GdipDrawPath(g_gfx, g_penEdge, edge); GdipDeletePath(edge); }
    GpPen* hi = NULL;
    if (GdipCreatePen1(ARGB(33, 255, 255, 255), 1.0f, GP_UNIT_PIXEL, &hi) == 0) {
        GdipDrawLineI(g_gfx, hi, L + S(18) + 2, T + 2, R - S(18) - 2, T + 2);
        GdipDeletePen(hi);
    }
...
static void panel_layout(HWND hwnd)
{
    ...
    // 间距模数（逻辑 px，全部 4 的倍数）：4/8/12/16/20
    int padX = S(16), padT = S(16), padB = S(12);
    ...
    if (g_mgr) {
        ...
        g_cell = S(96); g_gap = S(8);
        g_grid.left = g_panel.left + padX;
        g_grid.top = opY + S(30) + S(12);
        g_grid.right = g_panel.right - padX;
        g_footer.left   = g_panel.left + padX;   // 左右界此前从未设置（保持 0）：状态行/文件名曾从 x=0 起排、mgr 计数区为负宽
        ...
        // 速发模式三段式：检索行 → 标签胶囊行 → 格子阵列（首行=最近快捷格，同款 56px）
        // 检索行↔标签行 / 标签行↔最近行 间距对称（12/12）
        int chipY = g_searchBox.bottom + S(12);
        int ry = chipY + S(26) + S(12);
    ...
    // 悬停大图（右下）
    g_preview.right = g_panel.right - S(12);
...
static void draw_search_box(void)
{
    RECT rc = g_searchBox;
    int rad = S(11);
    // 聚焦态（有输入）：预渲染 accent 柔光拉伸至框 rect，peak 0.10
    if (g_search[0] || g_complen) draw_glow(g_glowFocus, rc, S(8));
    // 内凹暗底 + 细亮边（深色 UI 输入框惯例：凹陷感来自比面板更暗，而非白玻璃）
    GpSolidFill* bg = NULL;
    if (GdipCreateSolidFill(ARGB(82, 7, 9, 15), &bg) == 0) {
        fill_rr(rc.left, rc.top, rc.right, rc.bottom, rad, (GpBrush*)bg);
        GdipDeleteBrush((GpBrush*)bg);
    }
    GpPen* pen = NULL;
    u32 edge = g_search[0] || g_complen ? ARGB(40, 122, 162, 255) : ARGB(26, 255, 255, 255);
    ...
    // 放大镜（灰白弱化，不与文字抢焦点）
    GpPen* mp = NULL;
    if (GdipCreatePen1(C_INK(120), 1.6f, GP_UNIT_PIXEL, &mp) == 0) {
    ...
    // caret（空输入也显示，作为输入位置的引导）：1.8px 主体 + 两端 1px 收细近似圆头
    if (g_caretOn && !g_complen) {
        float preW = g_caret ? gtext_w(g_search, g_caret, g_fText) : 0.0f;
        int cxx = tx + (int)preW;
        int y0 = cy2 - S(8), y1 = cy2 + S(8);
        GpPen* cp = NULL;
        if (GdipCreatePen1(C_ACCENT(255), 1.8f, GP_UNIT_PIXEL, &cp) == 0) {
            GdipDrawLineI(g_gfx, cp, cxx, y0 + 1, cxx, y1 - 1);
            GdipDeletePen(cp);
        }
        GpPen* ct = NULL;
        if (GdipCreatePen1(C_ACCENT(215), 1.0f, GP_UNIT_PIXEL, &ct) == 0) {
            GdipDrawLineI(g_gfx, ct, cxx, y0, cxx, y0 + 1);
            GdipDrawLineI(g_gfx, ct, cxx, y1 - 1, cxx, y1);
            GdipDeletePen(ct);
        }
    }
    GdipResetClip(g_gfx);
...
{
    // 最近快捷行：与网格完全同款格子；无内容不画占位，不带数字角标（数字键已回归纯文本输入）
    for (int i = 0; i < 7; i++) {
        ...
        RECT dr = rc;
        if (hov) { dr.top -= S(HOV_LIFT); dr.bottom -= S(HOV_LIFT); }
        if (hov) draw_glow(g_glowSoft, rc, S(10));
        glass_cell(dr, S(11), hov ? 1 : 0, 0.030f);
...
static void draw_chip(int* px, int y, const wchar_t* txt, int id)
{
    ...
    gtext(txt, -1, g_fUi, rc, g_sfC,
          active ? ARGB(240, 238, 241, 255) : C_INK(hov ? 170 : 140));
    ...
    *px = rc.right + S(8);   // 胶囊间距 7 -> 8（4px 模数）
}
...
static void draw_empty(void)
{
    ...
    // 拖放区示意：虚线圆角矩形（GDI+ DashStyle）+ 向下箭头，纯矢量
    int bw = S(280), bh = S(104);
    RECT dz = { cx - bw / 2, cy - bh / 2, cx + bw / 2, cy + bh / 2 };
    GpPen* dp = NULL;
    if (GdipCreatePen1(C_INK(88), 1.2f, GP_UNIT_PIXEL, &dp) == 0) {
        GdipSetPenDashStyle(dp, GP_DASH);
        stroke_rr(dz.left, dz.top, dz.right, dz.bottom, S(12), dp);
        GdipDeletePen(dp);
    }
    GpPen* ap = NULL;
    if (GdipCreatePen1(C_ACCENT(195), 1.6f, GP_UNIT_PIXEL, &ap) == 0) {
        int ay1 = dz.top - S(12), ay0 = ay1 - S(26);
        GdipDrawLineI(g_gfx, ap, cx, ay0, cx, ay1);
        GdipDrawLineI(g_gfx, ap, cx - S(7), ay1 - S(8), cx, ay1);
        GdipDrawLineI(g_gfx, ap, cx, ay1, cx + S(7), ay1 - S(8));
        GdipDeletePen(ap);
    }
    gtext(L"把表情图拖到这里", -1, g_fUi,
          (RECT){ dz.left + S(8), cy - S(12), dz.right - S(8), cy + S(8) }, g_sfC, C_INK(135));
}
...
static void draw_grid(void)
{
    ...
    for (int k = 0; k < g_cols * g_rows; k++) {
        ...
        // 管理模式选中态（画在图之上，否则被 contain 满幅图盖住）：accent 描边 + 左上勾角标
        if (sel) {
            GpPen* pen = NULL;
        ...
        if (is_gif_name(e->name)) {
            RECT gc = { dr.right - S(26), dr.bottom - S(16), dr.right - S(4), dr.bottom - S(4) };
            GpSolidFill* bg = NULL;
            if (GdipCreateSolidFill(ARGB(185, 13, 15, 23), &bg) == 0) {
                fill_rr(gc.left, gc.top, gc.right, gc.bottom, S(4), (GpBrush*)bg);
                GdipDeleteBrush((GpBrush*)bg);
            }
            GpPen* gp2 = NULL;   // 1px 亮边：深底浮出的玻璃小牌
            if (GdipCreatePen1(ARGB(70, 255, 255, 255), 1.0f, GP_UNIT_PIXEL, &gp2) == 0) {
                stroke_rr(gc.left, gc.top, gc.right, gc.bottom, S(4), gp2);
                GdipDeletePen(gp2);
            }
            RECT gt = gc; gt.top -= S(1);
            gtext(L"GIF", -1, g_fTiny, gt, g_sfC, ARGB(232, 231, 235, 246));
        }
        // hover 名字条（高 22 / 文字 10px 单行省略：缩到 50% 仍可读）
        // scrim 用 7 段实心条近似垂直渐变：竖直轴 LineBrush 经验证不可靠（基线同样只渲染出 ±5/255）
        if (hov) {
            RECT nb = { dr.left, dr.bottom - S(22), dr.right, dr.bottom };
            GpPath* clip = rr_path(dr.left, dr.top, dr.right, dr.bottom, S(11));
            if (clip) GdipSetClipPath(g_gfx, clip, 0);
            const int NSTEP = 7;
            int sh2 = (nb.bottom - nb.top + NSTEP - 1) / NSTEP;
            for (int i = 0; i < NSTEP; i++) {
                int a = 24 + (206 - 24) * i / (NSTEP - 1);
                GpSolidFill* sb = NULL;
                if (GdipCreateSolidFill(ARGB(a, 8, 9, 14), &sb) == 0) {
                    int y0 = nb.top + i * sh2;
                    int y1 = (i == NSTEP - 1) ? nb.bottom : y0 + sh2;
                    GdipFillRectangleI(g_gfx, (GpBrush*)sb, nb.left, y0, nb.right - nb.left, y1 - y0);
                    GdipDeleteBrush((GpBrush*)sb);
                }
            }
            GdipResetClip(g_gfx);
            if (clip) GdipDeletePath(clip);
            RECT nt = nb; nt.bottom -= S(2);
            gtext(e->name, -1, g_fName, nt, g_sfNW, ARGB(255, 255, 255, 255));
        }
    }
    // 悬浮式滚动条：宽 4px、贴面板右缘内 2px、圆角 2、两端 10px alpha 渐隐、alpha 36
    int vis = g_cols * g_rows;
    if (g_nFilt > vis) {
        ...
        int tw = S(4);
        int tx = g_panel.right - S(2) - tw;
        int fadeL = S(10);
        if (thumbH > fadeL * 2 + S(8)) {
            GpSolidFill* bg = NULL;
            if (GdipCreateSolidFill(ARGB(36, 255, 255, 255), &bg) == 0) {
                fill_rr(tx, thumbY + fadeL, tx + tw, thumbY + thumbH - fadeL, S(2), (GpBrush*)bg);
                GdipDeleteBrush((GpBrush*)bg);
            }
            // 两端渐隐：各 3 段实心阶梯近似（竖直轴 LineBrush 不可靠，同名字条）
            const int fa[3] = { 8, 20, 32 };
            int cs = fadeL / 3;
            for (int i = 0; i < 3; i++) {
                GpSolidFill* c1 = NULL;
                if (GdipCreateSolidFill(ARGB(fa[i], 255, 255, 255), &c1) == 0) {
                    GdipFillRectangleI(g_gfx, (GpBrush*)c1, tx, thumbY + i * cs, tw, cs);
                    GdipDeleteBrush((GpBrush*)c1);
                }
                GpSolidFill* c2 = NULL;
                if (GdipCreateSolidFill(ARGB(fa[2 - i], 255, 255, 255), &c2) == 0) {
                    GdipFillRectangleI(g_gfx, (GpBrush*)c2, tx, thumbY + thumbH - fadeL + i * cs, tw, cs);
                    GdipDeleteBrush((GpBrush*)c2);
                }
            }
        } else {
            GpSolidFill* bg = NULL;
            if (GdipCreateSolidFill(ARGB(36, 255, 255, 255), &bg) == 0) {
                fill_rr(tx, thumbY, tx + tw, thumbY + thumbH, S(2), (GpBrush*)bg);
                GdipDeleteBrush((GpBrush*)bg);
            }
        }
    }
...
static void draw_footer(void)
{
    ...
    gtext(line, -1, g_fSmall, lr, g_sfL, C_INK(140));
}
...
static void draw_preview(void)
{
    ...
    RECT rc = g_preview;
    // 深底（α248）+ 双线框：外圈 1px 亮边 + 内圈 1px 暗分隔
    GpSolidFill* bg = NULL;
    if (GdipCreateSolidFill(ARGB(248, 13, 15, 23), &bg) == 0) {
    ...
    GpPen* dk = NULL;
    if (GdipCreatePen1(ARGB(95, 5, 7, 12), 1.0f, GP_UNIT_PIXEL, &dk) == 0) {
        stroke_rr(rc.left + 2, rc.top + 2, rc.right - 2, rc.bottom - 2, S(12), dk);
        GdipDeletePen(dk);
    }
    ...
    // 四角 L 形取景角标（1.5px，accent α180）：呼应「速发」的瞄准语义
    GpPen* vp = NULL;
    if (GdipCreatePen1(C_ACCENT(180), 1.5f, GP_UNIT_PIXEL, &vp) == 0) {
        int m = S(8), arm = S(10);
        int l = rc.left + m, t2 = rc.top + m, r2 = rc.right - m, b2 = rc.bottom - m;
        GdipDrawLineI(g_gfx, vp, l, t2 + arm, l, t2);                 // 左上
        GdipDrawLineI(g_gfx, vp, l, t2, l + arm, t2);
        GdipDrawLineI(g_gfx, vp, r2 - arm, t2, r2, t2);               // 右上
        GdipDrawLineI(g_gfx, vp, r2, t2, r2, t2 + arm);
        GdipDrawLineI(g_gfx, vp, l, b2 - arm, l, b2);                 // 左下
        GdipDrawLineI(g_gfx, vp, l, b2, l + arm, b2);
        GdipDrawLineI(g_gfx, vp, r2 - arm, b2, r2, b2);               // 右下
        GdipDrawLineI(g_gfx, vp, r2, b2 - arm, r2, b2);
        GdipDeletePen(vp);
    }
}
...
static void draw_mgr_button(int* pright, int y, const wchar_t* txt, int id, BOOL danger)
{
    ...
    if (on) {
        danger_pill(rc, S(15));   // 武装态：红色系专属渐变（语义色与 accent 分离）
    } else {
        glass_cell(rc, S(15), hov ? 1 : 0, 0.03f);
        if (danger && hov) {
            GpPen* pen = NULL;
    ...
    gtext(txt, -1, g_fUi, rc, g_sfC,
          on ? ARGB(255, 255, 240, 240) : C_INK(hov ? 190 : 150));
    g_btnRc[id] = rc;
    *pright = rc.left - S(8);
}
...
static void draw_mgr(void)
{
    ...
    // 胶囊高 26 / 按钮高 30：胶囊下移 2px 与按钮垂直居中对齐
    int opY = g_searchBox.bottom + S(12);
    int chipYm = opY + S(2);
    g_nChips = 0;
    int cx = g_panel.left + S(16);
    draw_chip(&cx, chipYm, L"全部", -1);
    for (int i = 0; i < MP_MAX_TAGS; i++)
        if (g_store.tagNames[i]) draw_chip(&cx, chipYm, g_store.tagNames[i], i);
    ...
    // 状态行：左反馈文案（185：可见性反馈，弱文案会淹没在深玻璃上）+ 右计数
    if (g_status[0])
...
static GpStringFormat* make_sf_nw(int h, int v)   // NoWrap：超宽单行省略，不折行
{
    GpStringFormat* sf = NULL;
    if (GdipCreateStringFormat(GP_NOWRAP, 0, &sf)) return NULL;
    GdipSetStringFormatAlign(sf, h);
    GdipSetStringFormatLineAlign(sf, v);
    GdipSetStringFormatTrimming(sf, GP_TRIM_ELLIPSIS);
    return sf;
...
void entry(void)
{
    ...
    g_fName  = make_font(fdc, S(10), FW_NORMAL,   L"Segoe UI");   // hover 名字条 10px
    g_fMono  = make_font(fdc, S(10), FW_NORMAL,   L"Consolas");
    // GIF 角标 9.5px：半磅字需 REAL emSize（LOGFONT 整数高度表达不了），走 family API
    if (GdipCreateFontFamilyFromName(L"Segoe UI", NULL, &g_fFam) == 0 && g_fFam)
        GdipCreateFont(g_fFam, 9.5f * g_dpi / 96.0f, 0, GP_UNIT_PIXEL, &g_fTiny);
    if (!g_fTiny) g_fTiny = make_font(fdc, S(9), FW_NORMAL, L"Segoe UI");   // 兜底
    ReleaseDC(NULL, fdc);
    ...
    g_sfNW = make_sf_nw(GP_ALIGN_CENTER, GP_ALIGN_CENTER);
    GdipCreatePen1(ARGB(31, 255, 255, 255), 1.0f, GP_UNIT_PIXEL, &g_penEdge);
    make_glow(&g_glowSoft, &g_glowSoftBuf, HOV_GLOW);      // hover 微光 0.34
    make_glow(&g_glowStrong, &g_glowStrongBuf, 0.38f);
    make_glow(&g_glowFocus, &g_glowFocusBuf, FOCUS_GLOW);  // 搜索聚焦 0.10
