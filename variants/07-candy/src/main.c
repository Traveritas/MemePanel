// main.c — MemePanel v0.4「果冻糖 CANDY」：紫夜底 + 按标签分配糖果色的果冻速发面板
// 架构：UpdateLayeredWindow 整窗提交 32bpp 预乘 DIB；GDI+（flat C，见 gp.h）负责全部
//       矢量/文字/位图合成（GDI 直写 DIB 会破坏 alpha 通道，故一律 GDI+）。
//       静态底图（紫夜垂直渐变 + 左上奶油柔光 + 右上草莓反光 + AA 奶油亮边）resize 时
//       预渲染一次；6 色糖果柔光贴图一次生成；果冻件（上弧提亮 + 底缘加深带 + 浅描边）
//       每帧矢量绘制；每帧 = memcpy 底图 + 动态件绘制 + 一次 ULW 提交。
...
// ---- 「果冻糖 CANDY」tokens（BRIEF.md）----
// 面板底：奶油暗紫 rgba(52,44,72,.94) 顶 -> rgba(38,32,54,.92) 底（紫夜，托起糖果色）
// 文字：主 #FFF8F0 奶油白 / 次 0.6；圆角：外壳 22 / 格子 16 / 胶囊与按钮 999（药丸）
// 强调色按标签分配糖果色（6 色板内循环）；「全部」/默认 = 薄荷
static const u8 CANDY6[6][3] = {
    { 255, 169,  77 },   // 0 蜜橙 #FFA94D
    { 255, 107, 157 },   // 1 草莓 #FF6B9D
    { 255, 212,  59 },   // 2 柠檬 #FFD43B
    { 116, 201, 255 },   // 3 苏打 #74C9FF
    { 183, 148, 255 },   // 4 葡萄 #B794FF
    {  77, 216, 168 },   // 5 薄荷 #4DD8A8
};
#define CANDY_MINT   5      // 「全部」/无激活时的默认糖果色
#define CANDY_STRAW  1      // 草莓（关闭按钮 hover / 删除武装）
#define INK_R 255           // 奶油白 #FFF8F0
#define INK_G 248
#define INK_B 240
...
#define C_INK(a)      ARGB(a, INK_R, INK_G, INK_B)
#define C_MINT(a)     ARGB(a, 77, 216, 168)
#define C_STRAW(a)    ARGB(a, 255, 107, 157)

// 糖果色分配：chipId<0（全部/默认）= 薄荷；否则按标签 id 在 6 色板内循环
static void candy_of(int chipId, int* r, int* g, int* b)
{
    int i = (chipId < 0) ? CANDY_MINT : (chipId % 6);
    *r = CANDY6[i][0]; *g = CANDY6[i][1]; *b = CANDY6[i][2];
}
...
// 质感件（一次生成）：6 色糖果板各一张径向柔光（激活胶囊 / 大图 / 武装态）
static GpBitmap* g_glowCandy[6];
static u8*       g_glowCandyBuf[6];

// 字体 / 字符串格式 / 笔刷
static GpFont* g_fText, * g_fUi, * g_fSmall, * g_fTiny, * g_fMono;
static GpStringFormat *g_sfL, * g_sfC, * g_sfR, * g_sfCF;
static GpPen* g_penEdge;      // 白 .12（面板边框）
...
// 当前激活标签的糖果色（无激活 = 薄荷）：hover 描边 / 大图 glow / footer 标签用
static void acc_now(int* r, int* g, int* b)
{
    if (g_tagFilter)
        for (int t = 0; t < MP_MAX_TAGS; t++)
            if (g_tagFilter & ((u64)1 << t)) { candy_of(t, r, g, b); return; }
    candy_of(-1, r, g, b);
}
static int acc_index(void)   // 预渲染 glow 图的板内下标
{
    if (g_tagFilter)
        for (int t = 0; t < MP_MAX_TAGS; t++)
            if (g_tagFilter & ((u64)1 << t)) return t % 6;
    return CANDY_MINT;
}

// 垂直渐变（逐 1px 实心条带实现）：本机 gdiplus 的 GdipCreateLineBrushI/I+F 均返回
// NotImplemented(6)（连基础变体的胶囊渐变也被它静默吞掉），故用等效条带替代，开销可忽略
static void vgrad_fill(RECT fill, const RECT* clipRc, int clipRad,
                       int aTop, int aBot, int r, int g, int b)
{
    int h = fill.bottom - fill.top;
    if (h < 1 || (aTop <= 0 && aBot <= 0)) return;
    GpPath* clip = NULL;
    if (clipRc) {
        clip = rr_path(clipRc->left, clipRc->top, clipRc->right, clipRc->bottom, clipRad);
        if (clip) GdipSetClipPath(g_gfx, clip, 0);
    }
    GpSolidFill* br = NULL;
    if (GdipCreateSolidFill(ARGB(1, r, g, b), &br) == 0) {
        for (int i = 0; i < h; i++) {
            int a = aTop + (aBot - aTop) * i / h;
            if (a <= 0) continue;
            if (a > 255) a = 255;
            GdipSetSolidFillColor(br, ARGB(a, r, g, b));
            GdipFillRectangleI(g_gfx, (GpBrush*)br, fill.left, fill.top + i,
                               fill.right - fill.left, 1);
        }
        GdipDeleteBrush((GpBrush*)br);
    }
    if (clip) { GdipResetClip(g_gfx); GdipDeletePath(clip); }
}

// 果冻上弧受光 + 底缘软影（clip 到圆角内）：
//   glossA：顶部 45% 白渐变峰值 alpha（0 = 不画）
//   bandA ：底部 4px 黑渐变峰值 alpha（0 = 不画），收在格子自身内、不外溢
static void rr_gloss(RECT rc, int rad, int glossA, int bandA)
{
    int h = rc.bottom - rc.top;
    if (glossA > 0) {
        RECT gr = rc;
        gr.bottom = rc.top + (int)(h * 0.45f) + 1;
        vgrad_fill(gr, &rc, rad, glossA, 0, 255, 255, 255);
    }
    if (bandA > 0) {
        int bh = S(4);
        if (bh > h) bh = h;
        RECT brr = rc;
        brr.top = rc.bottom - bh;
        vgrad_fill(brr, &rc, rad, 0, bandA, 0, 0, 0);
    }
}

// 果冻药丸（按钮 / 标签胶囊）：糖果色底 + 上弧提亮 + 底缘同色加深带 + 浅描边
// state: 0 常态 / 1 hover / 2 实底激活（白字）/ 3 按压内凹（顶暗底亮反转）
static void jelly_pill(RECT rc, int cr, int cg, int cb, int state)
{
    int h = rc.bottom - rc.top;
    int rad = h / 2;
    int solid = (state == 2);
    GpSolidFill* bg = NULL;
    if (GdipCreateSolidFill(solid ? ARGB(238, cr, cg, cb)
                                  : ARGB(state == 1 ? 106 : 66, cr, cg, cb), &bg) == 0) {
        fill_rr(rc.left, rc.top, rc.right, rc.bottom, rad, (GpBrush*)bg);
        GdipDeleteBrush((GpBrush*)bg);
    }
    if (state == 3) {   // 按压感：提亮消失 + 内凹
        GpPen* pd = NULL;
        if (GdipCreatePen1(ARGB(120, 0, 0, 0), 1.0f, GP_UNIT_PIXEL, &pd) == 0) {
            GdipDrawLineI(g_gfx, pd, rc.left + rad, rc.top + 1, rc.right - rad, rc.top + 1);
            GdipDeletePen(pd);
        }
        GpPen* pl = NULL;
        if (GdipCreatePen1(ARGB(80, 255, 248, 240), 1.0f, GP_UNIT_PIXEL, &pl) == 0) {
            GdipDrawLineI(g_gfx, pl, rc.left + rad, rc.bottom - 1, rc.right - rad, rc.bottom - 1);
            GdipDeletePen(pl);
        }
    } else {
        // 底缘同色加深带（暗 25%）
        ...
        if (clip) {
            GdipSetClipPath(g_gfx, clip, 0);
            GpSolidFill* bd = NULL;
            if (GdipCreateSolidFill(ARGB(solid ? 160 : 120, cr * 3 / 4, cg * 3 / 4, cb * 3 / 4), &bd) == 0) {
                GdipFillRectangleI(g_gfx, (GpBrush*)bd, rc.left, rc.bottom - S(3),
                                   rc.right - rc.left, S(3));
                GdipDeleteBrush((GpBrush*)bd);
            }
            GdipResetClip(g_gfx);
            GdipDeletePath(clip);
        }
        rr_gloss(rc, rad, solid ? 80 : (state == 1 ? 60 : 54), 0);
    }
    // 1.5px 描边：同色提亮 20%
    int lr = cr + (255 - cr) / 5, lg2 = cg + (255 - cg) / 5, lb = cb + (255 - cb) / 5;
    GpPen* pen = NULL;
    if (GdipCreatePen1(solid ? ARGB(255, lr, lg2, lb) : ARGB(state == 1 ? 215 : 150, cr, cg, cb),
                       1.5f, GP_UNIT_PIXEL, &pen) == 0) {
        stroke_rr(rc.left, rc.top, rc.right, rc.bottom, rad, pen);
        GdipDeletePen(pen);
    }
}

// 格子底（图下层）：常态奶油玻璃 α0.05；hover/选中略厚
static void cell_fill(RECT rc, int rad, int state)
{
    GpSolidFill* bg = NULL;
    if (GdipCreateSolidFill(state ? ARGB(30, INK_R, INK_G, INK_B) : ARGB(13, INK_R, INK_G, INK_B),
                            &bg) == 0) {
        fill_rr(rc.left, rc.top, rc.right, rc.bottom, rad, (GpBrush*)bg);
        GdipDeleteBrush((GpBrush*)bg);
    }
...
// 格子面（图上层）：hover = 果冻鼓起（提亮 + 底缘暗带 + 糖果色描边）；选中 = 内凹 + 描边
static void cell_shine(RECT rc, int rad, int state, int cr, int cg, int cb)
{
    if (state == 1) {
        rr_gloss(rc, rad, 46, 120);
        GpPen* pen = NULL;
        if (GdipCreatePen1(ARGB(235, cr, cg, cb), 1.5f, GP_UNIT_PIXEL, &pen) == 0) {
            stroke_rr(rc.left, rc.top, rc.right, rc.bottom, rad, pen);
            GdipDeletePen(pen);
        }
    } else if (state == 2) {
        ...
        GpPen* pl = NULL;
        if (GdipCreatePen1(ARGB(90, INK_R, INK_G, INK_B), 1.0f, GP_UNIT_PIXEL, &pl) == 0) {
            GdipDrawLineI(g_gfx, pl, rc.left + rad, rc.bottom - 1, rc.right - rad, rc.bottom - 1);
            GdipDeletePen(pl);
        }
        GpPen* pen = NULL;
        if (GdipCreatePen1(ARGB(250, cr, cg, cb), 2.0f, GP_UNIT_PIXEL, &pen) == 0) {
            stroke_rr(rc.left, rc.top, rc.right, rc.bottom, rad, pen);
            GdipDeletePen(pen);
        }
    } else {
        GpPen* pen = NULL;
        if (GdipCreatePen1(ARGB(30, INK_R, INK_G, INK_B), 1.0f, GP_UNIT_PIXEL, &pen) == 0) {
            stroke_rr(rc.left, rc.top, rc.right, rc.bottom, rad, pen);
            GdipDeletePen(pen);
        }
    }
...
// 果冻心（最近行「最常用」标识）：双圆瓣（中心相切，凹口深）+ 下三角尖 + 高光点，薄荷
static void draw_heart(int cx, int cy, int s)
{
    GpSolidFill* b = NULL;
    if (GdipCreateSolidFill(C_MINT(235), &b) == 0) {
        int r = s / 4;
        int lx = cx - s / 4, rx = cx + s / 4, ty = cy - s / 8;
        fill_rr(lx - r, ty - r, lx + r, ty + r, r, (GpBrush*)b);   // 左瓣
        fill_rr(rx - r, ty - r, rx + r, ty + r, r, (GpBrush*)b);   // 右瓣
        GpPath* tri = NULL;
        if (GdipCreatePath(GP_FILL_WINDING, &tri) == 0) {
            GdipAddPathLineI(tri, lx - r, ty, rx + r, ty);
            GdipAddPathLineI(tri, rx + r, ty, cx, cy + s / 2);
            GdipClosePathFigure(tri);
            GdipFillPath(g_gfx, (GpBrush*)b, tri);
            GdipDeletePath(tri);
        }
        // 高光点（果冻反光）
        int d = r;
        GpSolidFill* hw = NULL;
        if (GdipCreateSolidFill(ARGB(115, 255, 255, 255), &hw) == 0) {
            fill_rr(lx - r / 3, ty - r + r / 5, lx - r / 3 + d, ty - r + r / 5 + d, d / 2,
                    (GpBrush*)hw);
            GdipDeleteBrush((GpBrush*)hw);
        }
        GdipDeleteBrush((GpBrush*)b);
    }
}
...
// 静态件生成
// ============================================================
static void make_glow(GpBitmap** out, u8** bufOut, float peak, int r, int g, int b)
{
    const int N = 64;
    ...
    for (int y = 0; y < N; y++)
        for (int x = 0; x < N; x++) {
            ...
            if (a > 255) a = 255;
            px[y * N + x] = ARGB(a, r * a / 255, g * a / 255, b * a / 255);
        }
...
{
    for (int i = 0; i < 6; i++) {
        if (g_glowCandy[i])   { GdipDisposeImage((GpImage*)g_glowCandy[i]); g_glowCandy[i] = NULL; }
        if (g_glowCandyBuf[i]){ HeapFree(GetProcessHeap(), 0, g_glowCandyBuf[i]); g_glowCandyBuf[i] = NULL; }
    }
}

// 底图：紫夜垂直渐变 (52,44,72).94 -> (38,32,54).92 + 左上奶油柔光 + 右上极淡草莓反光
// + GDI+ AA 奶油亮边（用户决定：无投影阴影，四周纯透明）
static void gen_base(void)
{
    ...
    float rad = (float)S(22);
    float hw = pw / 2, hh = ph / 2;
    // 左上奶油柔光（0.06），右上草莓反光（0.03）：紫夜让糖果色跳出来的「夜灯」
    float hcx = L + pw * 0.14f, hcy = T - ph * 0.05f;
    float hrx = pw * 0.55f, hry = ph * 0.20f;
    float scx = L + pw * 0.88f, scy = T - ph * 0.10f;
    float srx = pw * 0.42f, sry = ph * 0.16f;
    ...
    for (int y = 0; y < H; y++) {
        ...
        for (int x = 0; x < W; x++) {
            ...
            float t = (fy - (float)T) / ph;
            if (t < 0) t = 0; if (t > 1) t = 1;
            float r = 52 + (38 - 52) * t, g = 44 + (32 - 44) * t, b = 72 + (54 - 72) * t;
            float ex = (fx - hcx) / hrx, ey = (fy - hcy) / hry;
            float e = ex * ex + ey * ey;
            if (e < 1) {                           // 奶油柔光：向暖白提亮
                float hl = 0.06f * (1 - e) * (1 - e);
                r += (INK_R - r) * hl; g += (INK_G - g) * hl; b += (INK_B - b) * hl;
            }
            float sx = (fx - scx) / srx, sy = (fy - scy) / sry;
            float se = sx * sx + sy * sy;
            if (se < 1) {                           // 草莓反光：右上极淡粉
                float hl = 0.03f * (1 - se) * (1 - se);
                r += (255 - r) * hl; g += (150 - g) * hl; b += (190 - b) * hl;
            }
            float aP = (0.94f + (0.92f - 0.94f) * t) * fmin_(0.7f - sd, 1.0f);  // 圆角边缘软化
            if (aP < 0) aP = 0;
            g_bits[(u64)y * W + x] =
                (((u32)(aP * 255)) << 24) |
    ...
    // 1.5px 奶油亮边 + inset 顶线（GDI+，AA）
    GpPath* edge = rr_path(L, T, R - 1, B - 1, S(22));
    if (edge) { GdipDrawPath(g_gfx, g_penEdge, edge); GdipDeletePath(edge); }
    GpPen* hi = NULL;
    if (GdipCreatePen1(ARGB(58, INK_R, INK_G, INK_B), 1.2f, GP_UNIT_PIXEL, &hi) == 0) {
        GdipDrawLineI(g_gfx, hi, L + S(22) + 2, T + 2, R - S(22) - 2, T + 2);
        GdipDeletePen(hi);
    }
...
static void panel_layout(HWND hwnd)
{
    ...
    if (g_mgr) {
        // 管理模式：操作行（胶囊 + 果冻药丸按钮）→ 大格网格 → 状态行
        int opY = g_searchBox.bottom + S(14);
        g_cell = S(96); g_gap = S(12);
        g_grid.left = g_panel.left + padX;
        g_grid.top = opY + S(32) + S(16);
        ...
        // 速发模式三段式：检索行 → 糖果药丸标签行 → 最近行（左端心形图标，64px 大圆格）→ 网格
        int chipY = g_searchBox.bottom + S(14);
        int ry = chipY + S(28) + S(16);
        int rcell = S(64);
        int rx0 = g_panel.left + padX + S(20) + S(12);   // 左端让位给「最常用」心形图标
        for (int i = 0; i < 7; i++) {
            g_recent[i].left = rx0 + i * (rcell + S(8));
            g_recent[i].right = g_recent[i].left + rcell;
            g_recent[i].top = ry;
            g_recent[i].bottom = ry + rcell;
        }
        g_cell = S(56); g_gap = S(8);
        g_grid.left = g_panel.left + padX;
        g_grid.top = ry + rcell + S(18);
        g_grid.right = g_panel.right - padX;
        g_footer.bottom = g_panel.bottom - padB;
        g_footer.top = g_footer.bottom - S(18);
        g_grid.bottom = g_footer.top - S(8);
    ...
    // footer 左右界（原版只设了 top/bottom，零宽会让 hover 文件名/管理状态行整条消失）
...
{
    RECT rc = g_searchBox;
    int rad = (rc.bottom - rc.top) / 2;   // 药丸 999
    // 内凹药丸：比紫夜面板更暗的紫黑 + 顶缘 1px 暗线 + 底缘 1px 亮线（凹陷感）
    GpSolidFill* bg = NULL;
    if (GdipCreateSolidFill(ARGB(200, 24, 19, 38), &bg) == 0) {
        fill_rr(rc.left, rc.top, rc.right, rc.bottom, rad, (GpBrush*)bg);
        GdipDeleteBrush((GpBrush*)bg);
    }
    GpPen* pen = NULL;
    u32 edge = g_search[0] || g_complen ? C_MINT(46) : ARGB(24, INK_R, INK_G, INK_B);
    if (GdipCreatePen1(edge, 1.0f, GP_UNIT_PIXEL, &pen) == 0) {
        stroke_rr(rc.left, rc.top, rc.right, rc.bottom, rad, pen);
        GdipDeletePen(pen);
    }
    GpPen* ind = NULL;
    if (GdipCreatePen1(ARGB(80, 0, 0, 0), 1.0f, GP_UNIT_PIXEL, &ind) == 0) {
        GdipDrawLineI(g_gfx, ind, rc.left + rad, rc.top + 1, rc.right - rad, rc.top + 1);
        GdipDeletePen(ind);
    }
    GpPen* inl = NULL;
    if (GdipCreatePen1(ARGB(44, INK_R, INK_G, INK_B), 1.0f, GP_UNIT_PIXEL, &inl) == 0) {
        GdipDrawLineI(g_gfx, inl, rc.left + rad, rc.bottom - 1, rc.right - rad, rc.bottom - 1);
        GdipDeletePen(inl);
    }

    // 糖果放大镜：薄荷圆 + 粗柄（2.2px，卡通）
    GpPen* mp = NULL;
    if (GdipCreatePen1(C_MINT(225), 2.2f, GP_UNIT_PIXEL, &mp) == 0) {
        int cx = rc.left + S(16), cy = (rc.top + rc.bottom) / 2 - S(1);
        GdipDrawEllipseI(g_gfx, mp, cx - S(5), cy - S(6), S(10), S(10));
        GdipDrawLineI(g_gfx, mp, cx + S(3), cy + S(4), cx + S(7), cy + S(8));
        GdipDeletePen(mp);
    }
    ...
    GdipSetClipRectI(g_gfx, tr.left - 2, tr.top, tr.right - tr.left + 4, rc.bottom - rc.top, 0);
    ...
    if (g_search[0] || g_complen) {
        ...
        if (g_complen) {
            ...
            GpPen* up = NULL;
            if (GdipCreatePen1(C_MINT(230), 1.4f, GP_UNIT_PIXEL, &up) == 0) {
    ...
    // caret（空输入也显示，作为输入位置的引导；薄荷糖果色）
    if (g_caretOn && !g_complen) {
        float preW = g_caret ? gtext_w(g_search, g_caret, g_fText) : 0.0f;
        int cxx = tx + (int)preW;
        GpPen* cp = NULL;
        if (GdipCreatePen1(C_MINT(255), 1.8f, GP_UNIT_PIXEL, &cp) == 0) {
    ...
    // 关闭按钮：圆形小果冻；hover = 草莓实底 + 白 X + 柔光
    RECT cb = g_closeRc;
    BOOL chov = PtInRect(&cb, g_mouse);
    int crad = (cb.bottom - cb.top) / 2;
    if (chov) draw_glow(g_glowCandy[CANDY_STRAW], cb, S(8));
    if (chov) {
        jelly_pill(cb, 255, 107, 157, 2);
    } else {
        GpSolidFill* cbg = NULL;
        if (GdipCreateSolidFill(ARGB(16, INK_R, INK_G, INK_B), &cbg) == 0) {
            fill_rr(cb.left, cb.top, cb.right, cb.bottom, crad, (GpBrush*)cbg);
            GdipDeleteBrush((GpBrush*)cbg);
        }
        GpPen* op = NULL;
        if (GdipCreatePen1(ARGB(30, INK_R, INK_G, INK_B), 1.0f, GP_UNIT_PIXEL, &op) == 0) {
            stroke_rr(cb.left, cb.top, cb.right, cb.bottom, crad, op);
            GdipDeletePen(op);
        }
    }
    GpPen* xp = NULL;
    if (GdipCreatePen1(chov ? ARGB(255, 255, 255, 255) : C_INK(105), 1.6f, GP_UNIT_PIXEL, &xp) == 0) {
        int m = S(9);
...
{
    // 最近快捷行：64px 大圆格（圆角 18），左端薄荷果冻心标识「最常用」
    int ar, ag, ab;
    acc_now(&ar, &ag, &ab);
    draw_heart(g_panel.left + S(16) + S(10),
               (g_recent[0].top + g_recent[0].bottom) / 2, S(20));
    for (int i = 0; i < 7; i++) {
        ...
        cell_fill(dr, S(18), hov ? 1 : 0);
        if (hov && g_anim.ei == ei && g_anim.pb) {
            draw_image_contain(g_anim.pb[g_anim.cur], g_anim.w, g_anim.h, dr, S(18));
            cell_shine(dr, S(18), 1, ar, ag, ab);
            continue;
        }
        int iw, ih;
        GpBitmap* pb = oneshot_get(&g_recentImg[i], ei, &iw, &ih);
        if (pb) draw_image_contain(pb, iw, ih, dr, S(18));
        cell_shine(dr, S(18), hov ? 1 : 0, ar, ag, ab);
    }
}
...
{
    int cr, cg, cb;
    candy_of(id, &cr, &cg, &cb);
    int active = (id == -1) ? (g_tagFilter == 0) : (g_tagFilter == ((u64)1 << id));
    int hov = 0;
    int h = S(28);
    // 激活时右侧留出计数徽章位
    float w = gtext_w(txt, -1, g_fUi) + S(30) + (active ? S(20) : 0);
    RECT rc = { *px, y, *px + (int)w, y + h };
    if (rc.right > g_panel.right - S(16)) return;    // 放不下的不画
    hov = PtInRect(&rc, g_mouse);

    if (active)
        draw_glow(g_glowCandy[(id < 0) ? CANDY_MINT : id % 6], rc, S(12));
    jelly_pill(rc, cr, cg, cb, active ? 2 : (hov ? 1 : 0));
    RECT tr = rc;
    if (active) tr.right -= S(18);                    // 文字让位徽章
    gtext(txt, -1, g_fUi, tr, g_sfC,
          active ? ARGB(255, 255, 255, 255) : C_INK(hov ? 210 : 188));

    if (active) {   // 糖果计数徽章：加深糖果色小圆 + 白字
        int cnt = 0;
        if (id < 0) cnt = g_store.count;
        else for (int i = 0; i < g_store.count; i++)
            if (g_store.entries[i].tagmask & ((u64)1 << id)) cnt++;
        int d = S(15);
        RECT bc = { rc.right - S(18), (rc.top + rc.bottom) / 2 - d / 2,
                    rc.right - S(18) + d, (rc.top + rc.bottom) / 2 + d / 2 };
        GpSolidFill* bb = NULL;
        if (GdipCreateSolidFill(ARGB(245, cr * 11 / 20, cg * 11 / 20, cb * 11 / 20), &bb) == 0) {
            fill_rr(bc.left, bc.top, bc.right, bc.bottom, d / 2, (GpBrush*)bb);
            GdipDeleteBrush((GpBrush*)bb);
        }
        wchar_t num[8];
        wnsprintfW(num, 8, L"%d", cnt);
        gtext(num, -1, g_fTiny, bc, g_sfC, ARGB(255, 255, 255, 255));
    }
...
static void draw_empty(void)
{
    ...
    gtext(L"把表情图拖到这里", -1, g_fUi,
          (RECT){ g_grid.left, cy - S(12), g_grid.right, cy + S(8) }, g_sfC, C_INK(130));
}
...
static void draw_grid(void)
{
    ...
    for (int k = 0; k < g_cols * g_rows; k++) {
        ...
        cell_fill(dr, rad, sel ? 2 : (hov ? 1 : 0));

        Entry* e = &g_store.entries[g_filt[idx]];
        int iw, ih;
        if (hov && g_anim.ei == g_filt[idx] && g_anim.pb) {
            // 悬停中的 GIF：播放动画帧（解码未就绪时走静态 fallback）
            draw_image_contain(g_anim.pb[g_anim.cur], g_anim.w, g_anim.h, dr, rad);
        } else {
            GpBitmap* pb = tc_get(idx, &iw, &ih);
            if (pb) draw_image_contain(pb, iw, ih, dr, rad);
        }
        if (is_gif_name(e->name)) {
            RECT gc = { dr.right - S(26), dr.bottom - S(16), dr.right - S(3), dr.bottom - S(3) };
            GpSolidFill* bg = NULL;
            if (GdipCreateSolidFill(ARGB(175, 26, 21, 40), &bg) == 0) {
                fill_rr(gc.left, gc.top, gc.right, gc.bottom, S(5), (GpBrush*)bg);
                GdipDeleteBrush((GpBrush*)bg);
            }
            RECT gt = gc; gt.top -= S(1);
            gtext(L"GIF", -1, g_fTiny, gt, g_sfC, C_INK(220));
        }
        // hover 名字条（紫夜纱底）
        if (hov) {
            RECT nb = { dr.left, dr.bottom - S(20), dr.right, dr.bottom };
            vgrad_fill(nb, &dr, rad, 0, 225, 20, 15, 34);
            RECT nt = nb; nt.bottom -= S(2);
            gtext(e->name, -1, g_fTiny, nt, g_sfC, ARGB(255, 255, 255, 255));
        }
        // 果冻面最后画：上弧提亮 + 底缘暗带 + 描边压在名字条上，鼓起感完整
        cell_shine(dr, rad, sel ? 2 : (hov ? 1 : 0), ar, ag, ab);

        // 管理模式选中态：糖果色勾选气泡（左上圆色块 + 白勾）
        if (sel) {
            int d = S(18);
            int kx = dr.left + S(6), ky = dr.top + S(6);
            GpSolidFill* kb = NULL;
            if (GdipCreateSolidFill(ARGB(242, ar, ag, ab), &kb) == 0) {
                fill_rr(kx, ky, kx + d, ky + d, d / 2, (GpBrush*)kb);
                GdipDeleteBrush((GpBrush*)kb);
            }
            GpPen* kp = NULL;
            if (GdipCreatePen1(ARGB(255, 255, 255, 255), 2.0f, GP_UNIT_PIXEL, &kp) == 0) {
                GdipDrawLineI(g_gfx, kp, kx + S(4), ky + S(9), kx + S(7), ky + S(12));
                GdipDrawLineI(g_gfx, kp, kx + S(7), ky + S(12), kx + S(14), ky + S(5));
                GdipDeletePen(kp);
            }
        }
    }
    ...
    if (g_nFilt > vis) {
        ...
        GpSolidFill* bg = NULL;
        if (GdipCreateSolidFill(ARGB(52, INK_R, INK_G, INK_B), &bg) == 0) {
...
{
    // 平时留白；仅悬停格子时显示「文件名 · 标签」——糖果色小药丸标签片（α200）
    ...
    int ar, ag, ab;
    acc_now(&ar, &ag, &ab);
    int tw = (int)gtext_w(line, -1, g_fSmall) + S(20);
    int maxW = rc.right - rc.left;
    if (tw > maxW) tw = maxW;
    RECT lr = { rc.left, rc.top + S(2), rc.left + tw, rc.bottom - S(2) };
    GpSolidFill* bg = NULL;
    if (GdipCreateSolidFill(ARGB(200, ar, ag, ab), &bg) == 0) {
        fill_rr(lr.left, lr.top, lr.right, lr.bottom, (lr.bottom - lr.top) / 2, (GpBrush*)bg);
        GdipDeleteBrush((GpBrush*)bg);
    }
    gtext(line, -1, g_fSmall, lr, g_sfC, ARGB(250, 255, 255, 255));
}
...
static void draw_preview(void)
{
    ...
    int rad = S(20);
    int bord = S(6);
    // 外圈糖果色柔光 + 6px 奶油白「糖纸边」+ 内衬紫夜深底（透底 PNG 也稳）
    draw_glow(g_glowCandy[acc_index()], rc, S(10));
    GpSolidFill* wf = NULL;
    if (GdipCreateSolidFill(ARGB(250, INK_R, INK_G, INK_B), &wf) == 0) {
        fill_rr(rc.left, rc.top, rc.right, rc.bottom, rad, (GpBrush*)wf);
        GdipDeleteBrush((GpBrush*)wf);
    }
    RECT ir = { rc.left + bord, rc.top + bord, rc.right - bord, rc.bottom - bord };
    GpSolidFill* ib = NULL;
    if (GdipCreateSolidFill(ARGB(235, 30, 24, 44), &ib) == 0) {
        fill_rr(ir.left, ir.top, ir.right, ir.bottom, rad - bord, (GpBrush*)ib);
        GdipDeleteBrush((GpBrush*)ib);
    }
    int ei = g_filt[g_previewIdx];
    if (g_anim.ei == ei && g_anim.pb) {
        draw_image_contain(g_anim.pb[g_anim.cur], g_anim.w, g_anim.h, ir, rad - bord);
        return;
    }
    int iw, ih;
    GpBitmap* pb = oneshot_get(&g_previewImg, ei, &iw, &ih);
    if (pb) draw_image_contain(pb, iw, ih, ir, rad - bord);
}

// 管理模式：操作行按钮（右端对齐排布，果冻药丸，rect 记录命中）
static void draw_mgr_button(int* pright, int y, const wchar_t* txt, int id, BOOL danger)
{
    ...
    float w = gtext_w(id == BTN_DELETE ? L"确认删除" : txt, -1, g_fUi) + S(26);
    RECT rc = { *pright - (int)w, y, *pright, y + S(32) };
    BOOL hov = PtInRect(&rc, g_mouse);
    BOOL on = (id == BTN_DELETE) ? g_delArm : FALSE;
    if (danger) {   // 删除：草莓系果冻；武装 = 草莓实底 + 白字 + 柔光
        if (on) draw_glow(g_glowCandy[CANDY_STRAW], rc, S(10));
        jelly_pill(rc, 255, 107, 157, on ? 2 : (hov ? 1 : 0));
        gtext(txt, -1, g_fUi, rc, g_sfC,
              on ? ARGB(255, 255, 255, 255) : (hov ? ARGB(235, 255, 200, 220) : ARGB(205, 255, 175, 205)));
    } else {        // 中性按钮：奶油果冻
        jelly_pill(rc, INK_R, INK_G, INK_B, hov ? 1 : 0);
        gtext(txt, -1, g_fUi, rc, g_sfC, C_INK(hov ? 205 : 165));
    }
    g_btnRc[id] = rc;
    *pright = rc.left - S(10);
}
...
{
    // 操作行：左糖果药丸（选中>0 时点击=批量打标）+ 右果冻按钮组
    int opY = g_searchBox.bottom + S(14);
    g_nChips = 0;
...
static void panel_repaint(void)
{
    ...
    if (g_mgr) {
        ...
        g_nChips = 0;
        int cx = g_panel.left + S(16);
        int chipY = g_searchBox.bottom + S(12);
...
static HICON make_app_icon(void)
{
    ...
    if (color && mask && colorBits && maskBits) {
        ...
        // 紫夜糖壳底：对角渐变 (58,48,84) -> (36,30,52)，手写像素
        u32* px = (u32*)colorBits;
        for (int y = 0; y < 64; y++)
            for (int x = 0; x < 64; x++) {
                float t = (x + y) / 126.0f;
                u8 r = (u8)(58 + (36 - 58) * t), g = (u8)(48 + (30 - 48) * t), b = (u8)(84 + (52 - 84) * t);
                px[y * 64 + x] = 0xFF000000u | (r << 16) | (g << 8) | b;   // alpha 修补：icon 走 ARGB 路径也正确
            }
        HDC mdc = CreateCompatibleDC(sdc);
        HGDIOBJ old = SelectObject(mdc, color);
        // 内部反渐变覆盖出圆角（用深紫圆角矩形裁形）
        HBRUSH glass = CreateSolidBrush(RGB(32, 26, 48));
        HBRUSH glass2 = CreateSolidBrush(RGB(58, 48, 84));
        HPEN edge = CreatePen(PS_SOLID, 3, RGB(77, 216, 168));   // 薄荷糖边
        ...
        SelectObject(mdc, ob);
        SelectObject(mdc, op);
        HBRUSH cheek = CreateSolidBrush(RGB(255, 140, 175));
        HGDIOBJ oc = SelectObject(mdc, cheek);
        Ellipse(mdc, 17, 32, 23, 37);
        Ellipse(mdc, 41, 32, 47, 37);
        SelectObject(mdc, oc);
        DeleteObject(cheek);
...
void entry(void)
{
    ...
    g_fUi    = make_font(fdc, S(13), FW_NORMAL,   L"Segoe UI");   // 标签字号略大（brief 12.5 -> 取 13）
    ...
    GdipCreatePen1(ARGB(46, INK_R, INK_G, INK_B), 1.5f, GP_UNIT_PIXEL, &g_penEdge);
    for (int i = 0; i < 6; i++)
        make_glow(&g_glowCandy[i], &g_glowCandyBuf[i], 0.22f,
                  CANDY6[i][0], CANDY6[i][1], CANDY6[i][2]);
