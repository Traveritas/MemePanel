...
enum {
    ...
    GP_WRAP_TILE      = 0,     // WrapModeTile（本机 GDI+ 对 Clamp 返回 InvalidParameter，
                               //  所有线渐变填充域均落在单个 tile 周期内，Tile 等价安全）
    ...
    GP_CM_INTERSECT   = 1,     // CombineModeIntersect
};
...
#define GP_LIST(GPA) \
    ...
    GPA(int,  GdipAddPathEllipseI,       (GpPath*, int x, int y, int width, int height)) \
    GPA(int,  GdipStartPathFigure,       (GpPath*)) \
    ...
    GPA(int,  GdipCreateLineBrush,       (const GpPointF*, const GpPointF*, u32 c1, u32 c2, int wrap, GpLineGradient**)) \
    GPA(int,  GdipCreateLineBrushFromRectI, (const GpRect*, u32 c1, u32 c2, int mode, int wrap, GpLineGradient**)) \
    ...
    GPA(int,  GdipFillEllipseI,          (GpGraphics*, GpBrush*, int x, int y, int width, int height)) \
